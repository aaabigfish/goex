...
func Stop() error {
	log.Infof("DispatchMgr shutting down...")
...
func (d *Dispatcher) start(workers, channelCapacity int) error {
	...
	if workers < 1 {
		log.Warnf(
			"DispatchMgr workers cannot be zero, using default value %d\n",
			DefaultMaxWorkers)
		workers = DefaultMaxWorkers
	}
	if channelCapacity < 1 {
		log.Warnf(
			"DispatchMgr jobs limit cannot be zero, using default values %d\n",
			DefaultJobsLimit)
...
func (d *Dispatcher) stop() error {
	...
	select {
	case <-ch:
		log.Debugf("DispatchMgr shutdown.")
		return nil
	case <-timer.C:
