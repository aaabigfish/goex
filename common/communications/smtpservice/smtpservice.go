...
func (s *SMTPservice) Setup(cfg *base.CommunicationsConfig) {
	...
	log.Debugf("CommunicationMgr SMTP: Setup - From: %v. To: %s. Server: %s.\n", s.From, s.RecipientList, s.Host)
}
...
// an internal event is triggered by GoEx
func (s *SMTPservice) Send(subject, msg string) error {
	...
	log.Debugf("CommunicationMgr SMTP: Sending email to %v. Subject: %s Message: %s [From: %s]\n", s.RecipientList,
		subject, msg, s.From)
