...
const (
	...
	getHelp = `Goex SlackBot, thank you for using this service!
...
func (s *Slack) NewConnection() error {
	if !s.Connected {
		...
		if s.Verbose {
			log.Debugf("CommunicationMgr Slack: %s [%s] connected to %s [%s] \nWebsocket URL: %s.\n",
				s.Details.Self.Name,
				s.Details.Self.ID,
				s.Details.Team.Domain,
				s.Details.Team.ID,
				s.Details.URL)
			log.Debugf("CommunicationMgr Slack: Public channels: %s\n", s.GetChannelsString())
		}
		...
		log.Debugf("CommunicationMgr Slack: Target channel ID: %v [#%v]\n", s.TargetChannelID,
			s.TargetChannel)
...
func (s *Slack) WebsocketReader() {
	for {
		...
		if err != nil {
			log.Errorf("CommunicationMgr err(%v)", err)
		}
		...
		case "pong":
			if s.Verbose {
				log.Debugf("CommunicationMgr Slack: Pong received from server")
			}
		default:
			log.Debugf("CommunicationMgr resp(%v)", string(resp))
		}
	}
}
...
func (s *Slack) handlePresenceChange(resp []byte) error {
	...
	if s.Verbose {
		log.Debugf("CommunicationMgr Slack: Presence change. User %s [%s] changed status to %s\n",
...
func (s *Slack) handleMessageResponse(resp []byte, data WebsocketResponse) error {
	...
	if s.Verbose {
		log.Debugf("CommunicationMgr Slack: Message received by %s [%s] with text: %s", s.GetUsernameByID(msg.User), msg.User, msg.Text)
	}
...
func (s *Slack) handleErrorResponse(data WebsocketResponse) error {
	if data.Error.Msg == "Socket URL has expired" {
		if s.Verbose {
			log.Debugf("CommunicationMgr Slack websocket URL has expired.. Reconnecting")
		}
...
func (s *Slack) handleHelloResponse() {
	if s.Verbose {
		log.Debugf("CommunicationMgr Slack: Websocket connected successfully.")
	}
	s.Connected = true
...
func (s *Slack) handleReconnectResponse(resp []byte) error {
	...
	if s.Verbose {
		log.Debugf("CommunicationMgr Slack: Reconnect URL set to %s", s.ReconnectURL)
	}
	return nil
}
...
func (s *Slack) WebsocketKeepAlive() {
	...
	for {
		...
		if err := s.WebsocketSend("ping", ""); err != nil {
			log.Errorf("CommunicationMgr Slack: WebsocketKeepAlive() error %s", err)
		}
	}
}
...
func (s *Slack) WebsocketSend(eventType, text string) error {
	...
	if s.Verbose {
		log.Debugf("CommunicationMgr Slack: Sending websocket message: %s", string(data))
	}
...
func (s *Slack) HandleMessage(msg *Message) error {
	...
	default:
		return s.WebsocketSend("message", "GoEx SlackBot - Command Unknown!")
	}
}
