...
const (
	...
	cmdHelpReply = `GoEx TelegramBot, thank you for using this service!
...
func (t *Telegram) Connect() error {
	...
	log.Debugf("Telegram: Connected successfully!")
	t.Connected = true
	go t.PollerStart()
	return nil
}
...
func (t *Telegram) PushEvent(event base.Event) error {
	...
	for user, ID := range t.AuthorisedClients {
		if ID == 0 {
			log.Warnf("Telegram: Unable to send message to %s as their ID isn't set. A user must issue any supported command to begin a session.\n", user)
			continue
		}
...
func (t *Telegram) InitialConnect() error {
		...
		if resp.Result[i].Message.From.UserName != "" && resp.Result[i].Message.From.ID != 0 {
			...
			if _, ok := t.AuthorisedClients[username]; !ok {
				if !knownBadUsers[username] {
					log.Warnf("Telegram: Received message from unauthorised user: %s\n", username)
	...
	for userName, ID := range t.AuthorisedClients {
		...
		err = t.SendMessage(fmt.Sprintf("GoEx bot has connected: Hello, %s!", userName), ID)
		if err != nil {
