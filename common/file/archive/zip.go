...
func UnZip(src, dest string) (fileList []string, err error) {
	...
	for x := range z.File {
		...
		if !strings.HasPrefix(fPath, filepath.Clean(dest)+string(os.PathSeparator)) {
			err = z.Close()
			if err != nil {
				log.Errorf(ErrUnableToCloseFile, z, err)
			}
		...
		if err != nil {
			...
			if errCls != nil {
				log.Errorf(ErrUnableToCloseFile, outFile, errCls)
			}
			return
		}
		...
		if errIOCopy != nil {
			...
			err = outFile.Close()
			if err != nil {
				log.Errorf(ErrUnableToCloseFile, outFile, err)
			}
			err = eFile.Close()
			if err != nil {
				log.Errorf(ErrUnableToCloseFile, eFile, err)
			}
...
func Zip(src, dest string) error {
	...
	if err != nil {
		...
		errCls := f.Close()
		if errCls != nil {
			log.Errorf("Global Failed to close file handle, manual deletion required: %v", errCls)
			return err
		}
		errRemove := os.Remove(dest)
		if errRemove != nil {
			log.Errorf("Global Failed to remove archive, manual deletion required: %v", errRemove)
		}
		return err
	}
...
func addFilesToZipWrapper(z *zip.Writer, src string, isDir bool) error {
	return filepath.Walk(src, func(path string, i os.FileInfo, err error) error {
		...
		_, err = io.Copy(w, f)
		if err != nil {
			log.Errorf("Global Failed to Copy data: %v", err)
		}
