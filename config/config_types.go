...
import (
	...
	"github.com/aaabigfish/goex/portfolio"
...
const (
	// GctScriptDefaultTimeoutValue default timeout value for virtual machines
	GctScriptDefaultTimeoutValue = 30 * time.Second
	// GctScriptDefaultMaxVirtualMachines max number of virtual machines that can be loaded at one time
	GctScriptDefaultMaxVirtualMachines uint8 = 10
)
...
var (
	...
	// GctScriptPath path to load/save scripts
	GctScriptPath string
)
...
type Config struct {
	...
	GCTScript            GctScriptConfig           `json:"gctscript"`
...
// Exchange holds all the information needed for each enabled goex.
