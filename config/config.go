...
import (
	...
	"github.com/aaabigfish/goex/common/connchecker"
	"github.com/aaabigfish/goex/common/convert"
	"github.com/aaabigfish/goex/common/file"
	"github.com/aaabigfish/goex/currency"
	"github.com/aaabigfish/goex/currency/forexprovider"
	"github.com/aaabigfish/goex/database"
	"github.com/aaabigfish/goex/portfolio/banking"
	...
	for i := range c.BankAccounts {
		if c.BankAccounts[i].Enabled {
			...
			if err != nil {
				...
				log.Warnf("ConfigMgr err(%v)", err.Error())
			}
		}
	}
}
...
func (c *Config) CheckCommunicationsConfig() {
	...
	if c.Communications.SMSGlobalConfig.Name == "" {
		...
		if len(c.Communications.SMSGlobalConfig.From) > 11 {
			log.Warnf("ConfigMgr SMSGlobal config supplied from name exceeds 11 characters, trimming.\n")
	...
	if c.Communications.SlackConfig.Name != "Slack" ||
		...
		log.Warnf("ConfigMgr Communications config name/s not set correctly")
	}
	if c.Communications.SlackConfig.Enabled {
		if c.Communications.SlackConfig.TargetChannel == "" ||
			...
			log.Warnf("ConfigMgr Slack enabled in config but variable data not set, disabling.")
		}
	}
	if c.Communications.SMSGlobalConfig.Enabled {
		if c.Communications.SMSGlobalConfig.Username == "" ||
			...
			log.Warnf("ConfigMgr SMSGlobal enabled in config but variable data not set, disabling.")
		}
	}
	if c.Communications.SMTPConfig.Enabled {
		if c.Communications.SMTPConfig.Host == "" ||
			...
			log.Warnf("ConfigMgr SMTP enabled in config but variable data not set, disabling.")
		}
	}
	if c.Communications.TelegramConfig.Enabled {
		if _, ok := c.Communications.TelegramConfig.AuthorisedClients["user_example"]; ok ||
			...
			log.Warnf("ConfigMgr Telegram enabled in config but variable data not set, disabling.")
		}
	}
}
...
func (c *Config) CheckPairConsistency(exchName string) error {
	...
	for x := range assetTypes {
		...
		log.Warnf("ConfigMgr Exchange %s: [%v] Removing enabled pair(s) %v from enabled pairs list, as it isn't located in the available pairs list.\n",
			exchName,
			assetTypes[x],
	...
	if !atLeastOneEnabled {
		...
		log.Warnf("ConfigMgr Exchange %s: [%v] No enabled pairs found in available pairs list, randomly added %v pair.",
			exchName,
			assetTypes[0],
			rPair)
	}
...
func (c *Config) CheckSyncManagerConfig() {
	...
	if c.SyncManagerConfig.TimeoutWebsocket <= 0 {
		log.Warnf("ConfigMgr Invalid sync manager websocket timeout value %v, defaulting to %v\n", c.SyncManagerConfig.TimeoutWebsocket, DefaultSyncerTimeoutWebsocket)
		c.SyncManagerConfig.TimeoutWebsocket = DefaultSyncerTimeoutWebsocket
	}
	if c.SyncManagerConfig.PairFormatDisplay == nil {
		log.Warnf("ConfigMgr Invalid sync manager pair format value %v, using default format eg BTC-USD\n", c.SyncManagerConfig.PairFormatDisplay)
	...
	if c.SyncManagerConfig.TimeoutREST <= 0 {
		log.Warnf("ConfigMgr Invalid sync manager REST timeout value %v, defaulting to %v\n", c.SyncManagerConfig.TimeoutREST, DefaultSyncerTimeoutREST)
		c.SyncManagerConfig.TimeoutREST = DefaultSyncerTimeoutREST
	}
	if c.SyncManagerConfig.NumWorkers <= 0 {
		log.Warnf("ConfigMgr Invalid sync manager worker count value %v, defaulting to %v\n", c.SyncManagerConfig.NumWorkers, DefaultSyncerWorkers)
		c.SyncManagerConfig.NumWorkers = DefaultSyncerWorkers
	}
	if c.SyncManagerConfig.FiatDisplayCurrency.IsEmpty() {
		log.Warnf("ConfigMgr Invalid sync manager fiat display currency value, defaulting to %v\n", currency.USD)
	...
	for i := range c.Exchanges {
		...
		if c.Exchanges[i].CurrencyPairs == nil {
			...
			if len(assets) == 0 {
				...
				log.Warnf("ConfigMgr %s no assets found, disabling...", c.Exchanges[i].Name)
				continue
			}
			...
			for index := range assets {
				...
				if err != nil {
					if errors.Is(err, currency.ErrAssetIsNil) {
						...
						log.Warnf("ConfigMgr Exchange %s: upgrading config for asset type %s and setting enabled.\n",
							c.Exchanges[i].Name,
							assets[index])
			...
			if !atLeastOne {
				...
				log.Warnf("ConfigMgr %s assets disabled, turning on asset %s",
					c.Exchanges[i].Name,
					assets[0])
		...
		if c.Exchanges[i].Enabled {
			if c.Exchanges[i].Name == "" {
				log.Errorf("ConfigMgr "+ErrExchangeNameEmpty, i)
				c.Exchanges[i].Enabled = false
				continue
			}
			if (c.Exchanges[i].API.AuthenticatedSupport || c.Exchanges[i].API.AuthenticatedWebsocketSupport) &&
				...
				if failed {
					...
					log.Warnf("ConfigMgr "+WarningExchangeAuthAPIDefaultOrEmptyValues, c.Exchanges[i].Name)
				}
			}
			if !c.Exchanges[i].Features.Supports.RESTCapabilities.AutoPairUpdates &&
				...
				if lastUpdated.Unix() <= time.Now().Unix() {
					log.Warnf("ConfigMgr "+WarningPairsLastUpdatedThresholdExceeded,
						c.Exchanges[i].Name,
						pairsLastUpdatedWarningThreshold)
				}
			}
			if c.Exchanges[i].HTTPTimeout <= 0 {
				log.Warnf("ConfigMgr Exchange %s HTTP Timeout value not set, defaulting to %v.\n",
					c.Exchanges[i].Name,
					defaultHTTPTimeout)
			...
			if c.Exchanges[i].WebsocketResponseCheckTimeout <= 0 {
				log.Warnf("ConfigMgr Exchange %s Websocket response check timeout value not set, defaulting to %v.",
					c.Exchanges[i].Name,
			...
			if c.Exchanges[i].WebsocketResponseMaxLimit <= 0 {
				log.Warnf("ConfigMgr Exchange %s Websocket response max limit value not set, defaulting to %v.",
					c.Exchanges[i].Name,
			...
			if c.Exchanges[i].WebsocketTrafficTimeout <= 0 {
				log.Warnf("ConfigMgr Exchange %s Websocket response traffic timeout value not set, defaulting to %v.",
					c.Exchanges[i].Name,
			...
			if c.Exchanges[i].Orderbook.WebsocketBufferLimit <= 0 {
				log.Warnf("ConfigMgr Exchange %s Websocket orderbook buffer limit value not set, defaulting to %v.",
					c.Exchanges[i].Name,
			...
			if c.Exchanges[i].Orderbook.PublishPeriod == nil || c.Exchanges[i].Orderbook.PublishPeriod.Nanoseconds() < 0 {
				log.Warnf("ConfigMgr Exchange %s Websocket orderbook publish period value not set, defaulting to %v.",
					c.Exchanges[i].Name,
			...
			if err != nil {
				log.Errorf("ConfigMgr Exchange %s: CheckPairConsistency error: %s\n",
					c.Exchanges[i].Name,
					err)
				c.Exchanges[i].Enabled = false
				continue
...
func (c *Config) CheckCurrencyConfigValues() error {
	...
	for x := range supported {
		if !c.forexProviderExists(supported[x]) {
			log.Warnf("ConfigMgr %s forex provider not found, adding to config...\n", supported[x])
	...
	for i := range c.Currency.ForexProviders {
		if !common.StringDataContainsInsensitive(supported, c.Currency.ForexProviders[i].Name) {
			log.Warnf("ConfigMgr %s forex provider not supported, please remove from config.\n",
	...
	if c.Currency.CurrencyFileUpdateDuration <= 0 {
		log.Warnf("ConfigMgr Currency file update duration invalid, defaulting to %s", currency.DefaultCurrencyFileDelay)
		c.Currency.CurrencyFileUpdateDuration = currency.DefaultCurrencyFileDelay
	}

	if c.Currency.ForeignExchangeUpdateDuration <= 0 {
		log.Warnf("ConfigMgr Currency foreign exchange update duration invalid, defaulting to %s", currency.DefaultForeignExchangeDelay)
...
func (c *Config) checkGCTScriptConfig() error {
	...
	if c.GCTScript.ScriptTimeout <= 0 {
		c.GCTScript.ScriptTimeout = GctScriptDefaultTimeoutValue
	}

	if c.GCTScript.MaxVirtualMachines == 0 {
		c.GCTScript.MaxVirtualMachines = GctScriptDefaultMaxVirtualMachines
	}
...
func (c *Config) CheckNTPConfig() {
	...
	if len(c.NTPClient.Pool) < 1 {
		log.Warnf("ConfigMgr NTPClient enabled with no servers configured, enabling default pool.")
...
func (c *Config) SetNTPCheck(input io.Reader) (string, error) {
	...
	log.Warnf("ConfigMgr Your system time is out of sync, this may cause issues with trading")
	log.Warnf("ConfigMgr How would you like to show future notifications? (a)lert at startup / (w)arn periodically / (d)isable")
	...
	for !answered {
		...
		case "d":
			...
			answered = true
		default:
			log.Warnf("ConfigMgr Invalid option selected, please try again (a)lert / (w)arn / (d)isable")
		}
	}
	return resp, nil
}
...
// MacOS/Linux: $HOME/.goex/config.json or config.dat
// Windows: %APPDATA%\GoEx\config.json or config.dat
...
func migrateConfig(configFile, targetDir string) (string, error) {
	...
	if file.Exists(target) {
		log.Warnf("ConfigMgr config file already found in '%s'; not overwriting, defaulting to %s", target, configFile)
		return configFile, nil
	}
...
func (c *Config) ReadConfigFromFile(configPath string, dryrun bool) error {
	...
	*c = *result
	...
	if c.EncryptConfig == fileEncryptionPrompt {
		...
		if err != nil {
			log.Errorf("ConfigMgr The encryption prompt failed, ignoring for now, next time we will prompt again. Error: %s\n", err)
			return nil
		}
		if confirm {
		...
		if err != nil {
			log.Errorf("ConfigMgr Cannot save config. Error: %s\n", err)
		}
	}
	return nil
}
...
func readEncryptedConfWithKey(reader *bufio.Reader, keyProvider func() ([]byte, error)) (*Config, error) {
	...
	for errCounter := 0; errCounter < maxAuthFailures; errCounter++ {
		key, err := keyProvider()
		if err != nil {
			log.Errorf("ConfigMgr PromptForConfigKey err: %s", err)
			continue
		}
		...
		if err != nil {
			log.Errorf("ConfigMgr Could not decrypt and deserialise data with given key. Invalid password?", err)
			continue
		}
		return c, nil
	}
...
func (c *Config) SaveConfigToFile(configPath string) error {
	...
	defer func() {
		if writer != nil {
			err = writer.Close()
			if err != nil {
				log.Errorf("ConfigMgr err(%v)", err)
			}
		}
	}()
...
func (c *Config) CheckConfig() error {
	...
	if err != nil {
		log.Errorf("DatabaseMgr Failed to configure database: %v", err)
	}
	...
	if err != nil {
		log.Errorf("ConfigMgr Failed to configure gctscript, feature has been disabled: %s", err)
	}
	...
	if c.GlobalHTTPTimeout <= 0 {
		log.Warnf("ConfigMgr Global HTTP Timeout value not set, defaulting to %v.", defaultHTTPTimeout)
