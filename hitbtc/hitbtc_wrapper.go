...
func (h *HitBTC) SetDefaults() {
	...
	err = h.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      apiURL,
		goex.WebsocketSpot: hitbtcWebsocketAddress,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
func (h *HitBTC) Run(ctx context.Context) {
	if h.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s (url: %s).", h.Name,
			common.IsEnabled(h.Websocket.IsEnabled()), hitbtcWebsocketAddress)
		h.PrintEnabledPairs()
	}
	...
	if !h.BypassConfigFormatUpgrades {
		...
		if err != nil {
			log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", h.Name, err)
			return
		}
		...
		if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
			...
			if err != nil {
				log.Errorf("ExchangeSys %s failed to update enabled currencies.", h.Name)
			}
		}
	}
...
// withdrawals
func (h *HitBTC) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	// TODO supported in v3 API
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (h *HitBTC) GetWithdrawalsHistory(_ context.Context, _ currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	// TODO supported in v3 API
	return nil, common.ErrFunctionNotSupported
}
...
func (h *HitBTC) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
...
func (h *HitBTC) GetOrderHistory(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for i := range allOrders {
		...
		side, err = order.StringToOrderSide(allOrders[i].Side)
		if err != nil {
			log.Errorf("ExchangeSys %s %v", h.Name, err)
		}
