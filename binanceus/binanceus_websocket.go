...
func (bi *Binanceus) WsConnect() error {
	...
	if bi.Websocket.CanUseAuthenticatedEndpoints() {
		...
		if err != nil {
			...
			log.Errorf("ExchangeSys %v unable to connect to authenticated Websocket. Error: %s", bi.Name, err)
		} else {
...
func (bi *Binanceus) KeepAuthKeyAlive() {
	...
	defer func() {
		...
		if er != nil {
			log.Errorf("WebsocketMgr %s closing user data stream error %v",
				bi.Name, er)
		}
	}()
	...
	for {
		...
		case <-ticks.C:
			...
			if err != nil {
				...
				log.Warnf("ExchangeSys " + bi.Name + " - Unable to renew auth websocket token, may experience shutdown")
			}
		}
	}
}
	...
	for y := range pairs {
		for z := range channels {
			...
			if len(subscriptions) >= 1023 {
				log.Warnf("WebsocketMgr BinanceUS has 1024 subscription limit, only subscribing within limit. Requested %v", len(pairs)*len(channels))
				break subs
			}
	...
	go func() {
		...
		for {
			...
			case j := <-bi.obm.jobs:
				...
				if err != nil {
					log.Errorf("WebsocketMgr %s processing websocket orderbook error %v", bi.Name, err)
				}
			}
		}
	}()
...
func (bi *Binanceus) applyBufferUpdate(pair currency.Pair) error {
	...
	if needsFetching {
		if bi.Verbose {
			log.Debugf("WebsocketMgr %s Orderbook: Fetching via REST", bi.Name)
		}
	...
	if err != nil {
		log.Errorf("WebsocketMgr %s error fetching recent orderbook when applying updates: %s", bi.Name, err)
	}

	if recent != nil {
		err = bi.obm.checkAndProcessUpdate(bi.ProcessUpdate, pair, recent)
		if err != nil {
			log.Errorf("WebsocketMgr %s error processing update - initiating new orderbook sync via REST: %s", bi.Name, err)
...
func (bi *Binanceus) flushAndCleanup(p currency.Pair) {
	...
	if errClean != nil {
		log.Errorf("WebsocketMgr %s flushing websocket error: %v", bi.Name, errClean)
	}
	errClean = bi.obm.cleanup(p)
	if errClean != nil {
		log.Errorf("WebsocketMgr %s cleanup websocket error: %v", bi.Name, errClean)
	}
}
