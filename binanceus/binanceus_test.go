...
func TestGetFeeByType(t *testing.T) {
	t.Parallel()
	sharedtestvalues.SkipTestIfCredentialsUnset(t, bi)
	if _, er := bi.GetFeeByType(context.Background(), &goex.FeeBuilder{
		IsMaker: true,
...
func TestGetFee(t *testing.T) {
	...
	var withdrawalFeeBuilder = &goex.FeeBuilder{
		Amount:        1,
		FeeType:       goex.CryptocurrencyWithdrawalFee,
		Pair:          currency.NewPair(currency.BTC, currency.LTC),
		PurchasePrice: 1,
	...
	var offlineFeeTradeBuilder = &goex.FeeBuilder{
		Amount:        1,
		FeeType:       goex.OfflineTradeFee,
		Pair:          currency.NewPair(currency.BTC, currency.LTC),
		PurchasePrice: 1,
	}
...
func TestGetTrades(t *testing.T) {
	t.Parallel()
	sharedtestvalues.SkipTestIfCredentialsUnset(t, bi)
	_, er := bi.GetMyTrades(context.Background(), &GetTradesParams{})
	if !errors.Is(er, errIncompleteArguments) {
		t.Errorf(" Binanceus GetTrades() expecting error %v, but found %v", errIncompleteArguments, er)
	}
	_, er = bi.GetMyTrades(context.Background(), &GetTradesParams{Symbol: "BTCUSDT"})
	if er != nil {
