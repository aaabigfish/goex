...
func (bi *Binanceus) SetDefaults() {
	...
	if err := bi.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:                   binanceusAPIURL,
		goex.RestSpotSupplementary:      binanceusAPIURL,
		goex.WebsocketSpot:              binanceusDefaultWebsocketURL,
		goex.WebsocketSpotSupplementary: binanceusDefaultWebsocketURL,
	}); err != nil {
		log.Errorf("ExchangeSys %s setting default endpoints error %v", bi.Name, err)
	}
	bi.Websocket = stream.New()
	bi.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	bi.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	bi.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (bi *Binanceus) Setup(exch *config.Exchange) error {
	...
	ePoint, err := bi.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (bi *Binanceus) Run(ctx context.Context) {
	if bi.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s.", bi.Name, common.IsEnabled(bi.Websocket.IsEnabled()))
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", bi.Name, err)
	}
}
...
func (bi *Binanceus) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (bi *Binanceus) GetWithdrawalsHistory(ctx context.Context, c currency.Code, a asset.Item) ([]goex.WithdrawalHistory, error) {
	if a != asset.Spot {
		return nil, fmt.Errorf("%w %v", asset.ErrNotSupported, a)
	}
...
func (bi *Binanceus) GetOrderInfo(ctx context.Context, orderID string, pair currency.Pair, assetType asset.Item) (*order.Detail, error) {
	...
	if err != nil {
		log.Errorf("ExchangeSys %s %v", bi.Name, err)
	}
...
func (bi *Binanceus) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
