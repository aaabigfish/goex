...
func (b *Bitmex) WsConnect() error {
	...
	if b.Verbose {
		log.Debugf("ExchangeSys Successfully connected to Bitmex %s at time: %s Limit: %d",
			welcomeResp.Info,
	...
	if _, ok := quickCapture["success"]; ok {
		...
		if decodedResp.Success {
			if len(quickCapture) == 3 {
				if b.Verbose {
					log.Debugf("ExchangeSys %s websocket: Successfully subscribed to %s",
						b.Name, decodedResp.Subscribe)
				}
			} else {
				b.Websocket.SetCanUseAuthenticatedEndpoints(true)
				if b.Verbose {
					log.Debugf("ExchangeSys %s websocket: Successfully authenticated websocket connection",
						b.Name)
				}
			}
			return nil
