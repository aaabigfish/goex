...
func (b *Bitmex) SetDefaults() {
	...
	err = b.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      bitmexAPIURL,
		goex.WebsocketSpot: bitmexWSURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
func (b *Bitmex) Setup(exch *config.Exchange) error {
	...
	wsEndpoint, err := b.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (b *Bitmex) Run(ctx context.Context) {
	if b.Verbose {
		...
		log.Debugf(
			"ExchangeSys %s Websocket: %s. (url: %s).\n",
			b.Name,
	...
	for x := range marketInfo {
		...
		case asset.PerpetualContract:
			if marketInfo[x].Typ == perpetualContractID {
				...
				if strings.Contains(marketInfo[x].Symbol, currency.UnderscoreDelimiter) {
					...
					if len(settlement) != 2 {
						log.Warnf("ExchangeSys %s currency %s %s cannot be added to tradable pairs",
							b.Name,
							marketInfo[x].Symbol,
							a)
						break
		...
		case asset.Futures:
			if marketInfo[x].Typ == futuresID {
				...
				if len(isolate[0]) < 3 {
					log.Warnf("ExchangeSys %s currency %s %s be cannot added to tradable pairs",
						b.Name,
						marketInfo[x].Symbol,
						a)
					break
...
// withdrawals
func (b *Bitmex) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	history, err := b.GetWalletHistory(ctx, "all")
	if err != nil {
		return nil, err
	}
	resp := make([]goex.FundingHistory, len(history))
	for i := range history {
...
func (b *Bitmex) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	history, err := b.GetWalletHistory(ctx, c.String())
	if err != nil {
		return nil, err
	}
	resp := make([]goex.WithdrawalHistory, len(history))
	for i := range history {
...
func (b *Bitmex) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
