...
func (b *BTCMarkets) SetDefaults() {
	...
	err = b.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      btcMarketsAPIURL,
		goex.WebsocketSpot: btcMarketsWSURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
func (b *BTCMarkets) Setup(exch *config.Exchange) error {
	...
	wsURL, err := b.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (b *BTCMarkets) Run(ctx context.Context) {
	if b.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s (url: %s).", b.Name, common.IsEnabled(b.Websocket.IsEnabled()), btcMarketsWSURL)
		b.PrintEnabledPairs()
	}
	...
	if err != nil {
		log.Errorf("ExchangeSys %s Failed to update enabled currencies Err:%s", b.Name, err)
		return
	}
	format, err := b.GetPairFormat(asset.Spot, false)
	if err != nil {
		log.Errorf("ExchangeSys %s Failed to update enabled currencies.", b.Name)
		return
	}
	...
	if forceUpdate {
		...
		log.Warnf("ExchangeSys "+goex.ResetConfigPairsWarningMessage, b.Name, asset.Spot, enabledPairs)
	...
	if err != nil {
		log.Errorf("ExchangeSys %s Failed to update order execution limits. Error: %v", b.Name, err)
	}
...
// withdrawals
func (b *BTCMarkets) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (b *BTCMarkets) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	withdrawals, err := b.ListWithdrawals(ctx, -1, -1, -1)
	if err != nil {
		return nil, err
	}
	resp := make([]goex.WithdrawalHistory, 0, len(withdrawals))
	for i := range withdrawals {
		if c.IsEmpty() || c.Equal(withdrawals[i].AssetName) {
			resp = append(resp, goex.WithdrawalHistory{
...
func (b *BTCMarkets) SubmitOrder(ctx context.Context, s *order.Submit) (*order.SubmitResponse, error) {
	...
	if tempResp.Amount != 0 {
		...
		if err != nil {
			log.Errorf("ExchangeSys Exchange %s: OrderID: %s base amount conversion error: %s\n", b.Name, submitResp.OrderID, err)
		}
	}
	...
	if tempResp.TargetAmount != 0 {
		...
		if err != nil {
			log.Errorf("ExchangeSys Exchange %s: OrderID: %s quote amount conversion error: %s\n", b.Name, submitResp.OrderID, err)
		}
	}
...
func (b *BTCMarkets) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
...
func (b *BTCMarkets) GetActiveOrders(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for x := range req.Pairs {
		...
		for y := range tempData {
			...
			default:
				log.Errorf("ExchangeSys %s unknown order type %s getting order", b.Name, tempData[y].Type)
			...
			default:
				log.Errorf("ExchangeSys %s unexpected status %s on order %v", b.Name, tempData[y].Status, tempData[y].OrderID)
