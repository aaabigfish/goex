...
func (by *Bybit) SetDefaults() {
	...
	err = by.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:         bybitAPIURL,
		goex.RestCoinMargined: bybitAPIURL,
		goex.RestUSDTMargined: bybitAPIURL,
		goex.RestFutures:      bybitAPIURL,
		goex.RestUSDCMargined: bybitAPIURL,
		goex.WebsocketSpot:    bybitWSBaseURL + wsSpotPublicTopicV2,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	...
	by.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	by.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	by.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (by *Bybit) Setup(exch *config.Exchange) error {
	...
	wsRunningEndpoint, err := by.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (by *Bybit) Run(ctx context.Context) {
	if by.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s.", by.Name, common.IsEnabled(by.Websocket.IsEnabled()))
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", by.Name, err)
	}
}
	...
	case asset.CoinMarginedFutures:
		...
		for x := range allPairs {
			...
			if len(contractSplit) != 2 {
				log.Warnf("ExchangeSys %s base currency %s cannot split contract name %s cannot add to tradable pairs",
					by.Name,
...
// withdrawals
func (by *Bybit) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (by *Bybit) GetWithdrawalsHistory(ctx context.Context, c currency.Code, a asset.Item) ([]goex.WithdrawalHistory, error) {
	switch a {
	case asset.CoinMarginedFutures:
		...
		withdrawHistory := make([]goex.WithdrawalHistory, len(w))
		for i := range w {
			withdrawHistory[i] = goex.WithdrawalHistory{
				Status:          w[i].Status,
...
func (by *Bybit) GetOrderHistory(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	case asset.Spot:
		...
		for i := range resp {
			...
			if err != nil {
				log.Errorf("ExchangeSys %s %v", by.Name, err)
			}
...
func (by *Bybit) GetFeeByType(_ context.Context, _ *goex.FeeBuilder) (float64, error) {
...
func (by *Bybit) UpdateOrderExecutionLimits(ctx context.Context, a asset.Item) error {
	...
	case asset.Spot:
		...
		for x := range pairsData {
			...
			if err != nil {
				log.Warnf("ExchangeSys %s unable to load limits for %v, pair data missing", by.Name, pairsData[x].Name)
				continue
			}
			if !enabled {
				continue
...
func (by *Bybit) GetFuturesContractDetails(ctx context.Context, item asset.Item) ([]futures.Contract, error) {
	...
	case asset.CoinMarginedFutures:
		...
		for i := range inverseContracts.List {
			...
			default:
				if by.Verbose {
					log.Warnf("ExchangeSys %v unhandled contract type for %v %v %v-%v", by.Name, item, cp, s, e)
				}
				ct = futures.Unknown
			}
