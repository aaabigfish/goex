...
func (p *Poloniex) processAccountPendingOrder(notification []interface{}) error {
	...
	if err != nil {
		...
		log.Errorf("WebsocketMgr %s - Unknown currency pair ID. Currency will appear as the pair ID: '%v'", p.Name, currencyID)
	}
...
func (p *Poloniex) processAccountOrderLimit(notification []interface{}) error {
	...
	if err != nil {
		...
		log.Errorf(
			"WebsocketMgr %s - Unknown currency pair ID. Currency will appear as the pair ID: '%v'",
			p.Name,
			currencyID)
	}
