...
func (p *Poloniex) SetDefaults() {
	...
	err = p.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:              poloniexAPIURL,
		goex.RestSpotSupplementary: poloniexAltAPIUrl,
		goex.WebsocketSpot:         poloniexWebsocketAddress,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	p.Websocket = stream.New()
	p.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	p.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	p.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (p *Poloniex) Setup(exch *config.Exchange) error {
	...
	wsRunningURL, err := p.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (p *Poloniex) Run(ctx context.Context) {
	if p.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s (url: %s).\n",
			p.Name,
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s",
			p.Name,
			err)
		return
	}
	...
	if common.StringDataCompare(avail.Strings(), "BTC_USDT") {
		log.Warnf("ExchangeSys %s contains invalid pair, forcing upgrade of available currencies.\n",
			p.Name)
		forceUpdate = true
	}
...
// withdrawals
func (p *Poloniex) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	end := time.Now()
	walletActivity, err := p.WalletActivity(ctx, end.Add(-time.Hour*24*365), end, "")
	if err != nil {
		return nil, err
	}
	resp := make([]goex.FundingHistory, len(walletActivity.Deposits))
...
func (p *Poloniex) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	end := time.Now()
	withdrawals, err := p.WalletActivity(ctx, end.Add(-time.Hour*24*365), end, "withdrawals")
	if err != nil {
		return nil, err
	}
	resp := make([]goex.WithdrawalHistory, 0, len(withdrawals.Withdrawals))
...
func (p *Poloniex) GetOrderInfo(ctx context.Context, orderID string, pair currency.Pair, _ asset.Item) (*order.Detail, error) {
	...
	if orderInfo.Status, err = order.StringToOrderStatus(resp.Status); err != nil {
		log.Errorf("ExchangeSys %s %v", p.Name, err)
	}
...
func (p *Poloniex) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
...
func (p *Poloniex) GetActiveOrders(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for key := range resp.Data {
		...
		for i := range resp.Data[key] {
			...
			if err != nil {
				log.Errorf("ExchangeSys Exchange %v Func %v Order %v Could not parse date to unix with value of %v",
					p.Name,
					"GetActiveOrders",
