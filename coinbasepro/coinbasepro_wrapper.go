...
func NewCoinbasePro() *CoinbasePro {
	b := new(CoinbasePro)
	b.SetDefaults()
...
func (c *CoinbasePro) SetDefaults() {
	...
	c.Features = goex.Features{
		Supports: goex.FeaturesSupported{
			...
			WithdrawPermissions: goex.AutoWithdrawCryptoWithAPIPermission |
				goex.AutoWithdrawFiatWithAPIPermission,
	...
	err = c.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      coinbaseproAPIURL,
		goex.RestSandbox:   coinbaseproSandboxAPIURL,
		goex.WebsocketSpot: coinbaseproWebsocketURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	c.Websocket = stream.New()
	c.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	c.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	c.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (c *CoinbasePro) Setup(exch *config.Exchange) error {
	...
	wsRunningURL, err := c.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (c *CoinbasePro) Run(ctx context.Context) {
	...
	if !c.BypassConfigFormatUpgrades {
		...
		if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
			...
			if err != nil {
				...
				forceUpdate = true
				log.Warnf("ExchangeSys "+goex.ResetConfigPairsWarningMessage, c.Name, asset.Spot, p)
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", c.Name, err)
	}
}
...
// withdrawals
func (c *CoinbasePro) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (c *CoinbasePro) GetWithdrawalsHistory(_ context.Context, _ currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
...
func (c *CoinbasePro) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
...
func (c *CoinbasePro) GetActiveOrders(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for i := range respOrders {
		...
		if err != nil {
			log.Errorf("ExchangeSys %s %v", c.Name, err)
		}
