...
func (e *EXMO) SetDefaults() {
	...
	err = e.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot: exmoAPIURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
func (e *EXMO) Run(ctx context.Context) {
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", e.Name, err)
	}
}
...
// withdrawals
func (e *EXMO) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	hist, err := e.GetWalletHistory(ctx, 0)
	if err != nil {
		return nil, err
	}
	resp := make([]goex.FundingHistory, 0, len(hist.History))
...
func (e *EXMO) GetWithdrawalsHistory(ctx context.Context, _ currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	hist, err := e.GetWalletHistory(ctx, 0)
	if err != nil {
		return nil, err
	}
	resp := make([]goex.WithdrawalHistory, 0, len(hist.History))
	for i := range hist.History {
...
func (e *EXMO) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
