...
func (i *ItBit) SetDefaults() {
	...
	err = i.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot: itbitAPIURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
// withdrawals
func (i *ItBit) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (i *ItBit) GetWithdrawalsHistory(_ context.Context, _ currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
...
func (i *ItBit) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
...
func (i *ItBit) GetActiveOrders(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for j := range allOrders {
		...
		if err != nil {
			log.Errorf("ExchangeSys %s %v", i.Name, err)
		}
