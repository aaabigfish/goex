...
func (b *Bitstamp) WsConnect() error {
	...
	if b.Verbose {
		log.Debugf("ExchangeSys %s Connected to Websocket.\n", b.Name)
	}
...
func (b *Bitstamp) wsHandleData(respRaw []byte) error {
	...
	case "bts:subscribe", "bts:subscription_succeeded":
		if b.Verbose {
			log.Debugf("ExchangeSys %v - Websocket subscription acknowledgement", b.Name)
		}
	case "bts:unsubscribe":
		if b.Verbose {
			log.Debugf("ExchangeSys %v - Websocket unsubscribe acknowledgement", b.Name)
		}
	case "bts:request_reconnect":
		if b.Verbose {
			log.Debugf("ExchangeSys %v - Websocket reconnection request received", b.Name)
		}
		go func() {
			err := b.Websocket.Shutdown()
			if err != nil {
				log.Errorf("WebsocketMgr %s failed to shutdown websocket: %v", b.Name, err)
			}
...
func (b *Bitstamp) FetchWSAuth(ctx context.Context) (*WebsocketAuthResponse, error) {
	...
	err := b.SendAuthenticatedHTTPRequest(ctx, goex.RestSpot, bitstampAPIWSAuthToken, true, nil, resp)
	if err != nil {
