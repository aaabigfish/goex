...
func (b *Bitstamp) SetDefaults() {
	...
	err = b.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      bitstampAPIURL,
		goex.WebsocketSpot: bitstampWSURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
func (b *Bitstamp) Run(ctx context.Context) {
	...
	if !b.BypassConfigFormatUpgrades {
		...
		if err != nil {
			log.Errorf("ExchangeSys %s failed to get pair format. Err %s\n",
				b.Name,
				err)
			return
		}
		...
		if err != nil {
			log.Errorf("ExchangeSys %s failed to get enabled currencies. Err %s\n",
				b.Name,
				err)
			return
		}
		...
		if err != nil {
			log.Errorf("ExchangeSys %s failed to get available currencies. Err %s\n",
				b.Name,
				err)
			return
		}
		...
		if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
			...
			if err != nil {
				...
				log.Warnf(
					"ExchangeSys "+goex.ResetConfigPairsWarningMessage, b.Name, asset.Spot, enabledPairs)
				forceUpdate = true
...
func (b *Bitstamp) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
...
// withdrawals
func (b *Bitstamp) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (b *Bitstamp) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
...
func (b *Bitstamp) GetActiveOrders(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for i := range resp {
		...
		if err != nil {
			log.Errorf(
				"ExchangeSys %s GetActiveOrders unable to parse time: %s\n", b.Name, err)
		}
...
func (b *Bitstamp) GetOrderHistory(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for i := range resp {
		...
		default:
			log.Warnf(
				"ExchangeSys %s No base currency found for ID '%d'\n",
				b.Name,
				resp[i].OrderID)
		}
		...
		default:
			log.Warnf(
				"ExchangeSys %s No quote currency found for orderID '%d'\n",
				b.Name,
				resp[i].OrderID)
		}
		...
		if err != nil {
			log.Errorf(
				"ExchangeSys %s GetOrderHistory unable to parse time: %s\n", b.Name, err)
		}
