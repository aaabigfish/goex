...
func (g *Gateio) SetDefaults() {
	...
	err = g.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:              gateioTradeURL,
		goex.RestFutures:           gateioFuturesLiveTradingAlternative,
		goex.RestSpotSupplementary: gateioFuturesTestnetTrading,
		goex.WebsocketSpot:         gateioWebsocketEndpoint,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
// withdrawals
func (g *Gateio) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (g *Gateio) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	records, err := g.GetWithdrawalRecords(ctx, c, time.Time{}, time.Time{}, 0, 0)
	if err != nil {
		return nil, err
	}
	withdrawalHistories := make([]goex.WithdrawalHistory, len(records))
	for x := range records {
		withdrawalHistories[x] = goex.WithdrawalHistory{
...
func (g *Gateio) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
	...
	case asset.Spot, asset.Margin, asset.CrossMargin:
		...
		for x := range spotOrders {
			...
			for y := range spotOrders[x].Orders {
				...
				if err != nil {
					log.Errorf("ExchangeSys %s %v", g.Name, err)
				}
