...
import (
	...
	"github.com/aaabigfish/goex/collateral"
	"github.com/aaabigfish/goex/common"
	"github.com/aaabigfish/goex/config"
	"github.com/aaabigfish/goex/currency"
	"github.com/aaabigfish/goex/deposit"
	"github.com/aaabigfish/goex/fundingrate"
	"github.com/aaabigfish/goex/futures"
	"github.com/aaabigfish/goex/kline"
	"github.com/aaabigfish/goex/margin"
	"github.com/aaabigfish/goex/order"
	"github.com/aaabigfish/goex/orderbook"
	"github.com/aaabigfish/goex/portfolio/withdraw"
	"github.com/aaabigfish/goex/protocol"
	"github.com/aaabigfish/goex/request"
	"github.com/aaabigfish/goex/stream"
	"github.com/aaabigfish/goex/stream/buffer"
	"github.com/aaabigfish/goex/ticker"
	"github.com/aaabigfish/goex/trade"
...
func (b *Binance) SetDefaults() {
	...
	b.Features = goex.Features{
		Supports: goex.FeaturesSupported{
			REST:                true,
			Websocket:           true,
			...
			WithdrawPermissions: goex.AutoWithdrawCrypto |
				goex.NoFiatWithdrawals,
			Kline: kline.ExchangeCapabilitiesSupported{
				DateRanges: true,
				Intervals:  true,
			},
			FuturesCapabilities: goex.FuturesCapabilities{
				Positions:            true,
				Leverage:             true,
				CollateralMode:       true,
				FundingRates:         true,
	...
	b.Requester, err = request.New(b.Name,
		common.NewHTTPClientWithTimeout(goex.DefaultHTTPTimeout),
		request.WithLimiter(SetRateLimit()))
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	b.API.Endpoints = b.NewEndpoints()
	err = b.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:              spotAPIURL,
		goex.RestSpotSupplementary: apiURL,
		goex.RestUSDTMargined:      ufuturesAPIURL,
		goex.RestCoinMargined:      cfuturesAPIURL,
		goex.EdgeCase1:             "https://www.binance.com",
		goex.WebsocketSpot:         binanceDefaultWebsocketURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	...
	b.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	b.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
}
...
func (b *Binance) Setup(exch *config.Exchange) error {
	...
	ePoint, err := b.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (b *Binance) Run(ctx context.Context) {
	if b.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s. (url: %s).", b.Name,
	...
	for x := range a {
		if err := b.UpdateOrderExecutionLimits(ctx, a[x]); err != nil {
			log.Errorf("ExchangeSys %s failed to set exchange order execution limits. Err: %v", b.Name, err)
		}
		if a[x] == asset.USDTMarginedFutures && !b.BypassConfigFormatUpgrades {
			format, err := b.GetPairFormat(asset.USDTMarginedFutures, false)
			if err != nil {
				log.Errorf("ExchangeSys %s failed to get enabled currencies. Err %s", b.Name, err)
				return
			}
			...
			if err != nil {
				log.Errorf("ExchangeSys %s failed to get available currencies. Err %s", b.Name, err)
				return
			}
			if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
				...
				if err != nil {
					log.Errorf("ExchangeSys %s failed to update currencies. Err %s\n", b.Name, err)
				} else {
					log.Warnf(goex.ResetConfigPairsWarningMessage, b.Name, a[x], enabledPairs)
					forceUpdate = true
		...
		for i := range pairs {
			for y := range tick {
				...
				err = ticker.ProcessTicker(&ticker.Price{
					...
					PriceChange:        tick[y].PriceChange,
					PriceChangePercent: tick[y].PriceChangePercent,
					Pair:               pairFmt,
					ExchangeName:       b.Name,
					AssetType:          a,
				})
...
func (b *Binance) UpdateTicker(ctx context.Context, p currency.Pair, a asset.Item) (*ticker.Price, error) {
	...
	case asset.USDTMarginedFutures:
		...
		err = ticker.ProcessTicker(&ticker.Price{
			...
			PriceChange:        tick[0].PriceChange,
			PriceChangePercent: tick[0].PriceChangePercent,
			Pair:               p,
			ExchangeName:       b.Name,
			AssetType:          a,
		})
...
// withdrawals
func (b *Binance) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (b *Binance) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	withdrawals, err := b.WithdrawHistory(ctx, c, "", time.Time{}, time.Time{}, 0, 10000)
	if err != nil {
		return nil, err
	}

	resp := make([]goex.WithdrawalHistory, len(withdrawals))
	for i := range withdrawals {
		resp[i] = goex.WithdrawalHistory{
...
func (b *Binance) GetOrderInfo(ctx context.Context, orderID string, pair currency.Pair, assetType asset.Item) (*order.Detail, error) {
	...
	case asset.Spot:
		...
		if err != nil {
			log.Errorf("ExchangeSys %s %v", b.Name, err)
		}
		...
		pair.Delimiter = ""
		trade, err := b.GetMyTrades(context.Background(), &GetTradesParams{
			Symbol:  pair.String(),
		...
		var feeAsset string
		if len(trade) > 0 {
			fee = trade[0].Commission
			feeAsset = trade[0].CommissionAsset
		}
		...
		detail := &order.Detail{
			...
			FeeAsset:       currency.NewCode(feeAsset),
			Fee:            fee,
			Date:           resp.Time,
	...
	case asset.CoinMarginedFutures:
		...
		var feeBuilder goex.FeeBuilder
...
func (b *Binance) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
		return 0, fmt.Errorf("%T %w", feeBuilder, common.ErrNilPointer)
	}
	if (!b.AreCredentialsValid(ctx) || b.SkipAuthCheck) && // Todo check connection status
		feeBuilder.FeeType == goex.CryptocurrencyTradeFee {
		feeBuilder.FeeType = goex.OfflineTradeFee
	}
