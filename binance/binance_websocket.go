...
func (b *Binance) WsConnect() error {
	...
	if b.Websocket.CanUseAuthenticatedEndpoints() {
		...
		if err != nil {
			...
			log.Errorf("ExchangeSys %v unable to connect to authenticated Websocket. Error: %s", b.Name, err)
		} else {
			...
			if err != nil {
				log.Errorf("ExchangeSys %v SetWebsocketURL(%s). Error: %s", b.Name, authPayload, err)
				return err
			}
		}
	}
	...
	for {
		...
		case <-ticks.C:
			...
			if err != nil {
				...
				log.Warnf("ExchangeSys %s - Unable to renew auth websocket token, may experience shutdown", b.Name)
			}
		}
	}
}
...
func (b *Binance) applyBufferUpdate(pair currency.Pair) error {
	...
	if needsFetching {
		if b.Verbose {
			...
			log.Debugf("WebsocketMgr %s Orderbook: Fetching via REST", b.Name)
		}
	...
	if err != nil {
		log.Errorf("WebsocketMgr %s error fetching recent orderbook when applying updates: %s", b.Name, err)
	}

	if recent != nil {
		err = b.obm.checkAndProcessUpdate(b.ProcessUpdate, pair, recent)
		if err != nil {
			log.Errorf("WebsocketMgr %s error processing update - initiating new orderbook sync via REST: %s", b.Name, err)
	...
	go func() {
		...
		for {
			...
			case j := <-b.obm.jobs:
				...
				if err != nil {
					log.Errorf("WebsocketMgr %s processing websocket orderbook error %v", b.Name, err)
				}
			}
		}
	}()
...
func (b *Binance) flushAndCleanup(p currency.Pair) {
	...
	if errClean != nil {
		log.Errorf("WebsocketMgr %s flushing websocket error: %v", b.Name, errClean)
	}
	errClean = b.obm.cleanup(p)
	if errClean != nil {
		log.Errorf("WebsocketMgr %s cleanup websocket error: %v", b.Name, errClean)
	}
}
