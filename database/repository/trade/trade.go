...
import (
	...
	"github.com/aaabigfish/goex/database/models/postgres"
	"github.com/aaabigfish/goex/database/models/sqlite3"
	"github.com/aaabigfish/goex/database/repository"
	"github.com/aaabigfish/goex/database/repository/exchange"
...
func Insert(trades ...Data) error {
	...
	defer func() {
		if err != nil {
			errRB := tx.Rollback()
			if errRB != nil {
				log.Errorf("Insert tx.Rollback %v", errRB)
			}
		}
	}()
...
func DeleteTrades(trades ...Data) error {
	...
	defer func() {
		if err != nil {
			errRB := tx.Rollback()
			if errRB != nil {
				log.Errorf("DeleteTrades tx.Rollback %v", errRB)
			}
		}
	}()
	if repository.GetSQLDialect() == database.DBSQLite3 || repository.GetSQLDialect() == database.DBSQLite {
