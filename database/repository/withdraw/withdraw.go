...
import (
	...
	exchangeDB "github.com/aaabigfish/goex/database/repository/exchange"
...
func Event(res *withdraw.Response) {
	...
	if err != nil {
		log.Errorf("Event transaction being failed: %v", err)
		return
	}
	...
	if err != nil {
		log.Errorf("Event insert failed: %v", err)
		err = tx.Rollback()
		if err != nil {
			log.Errorf("Event Transaction rollback failed: %v", err)
		}
		return
	}
	...
	err = tx.Commit()
	if err != nil {
		log.Errorf("Event Transaction commit failed: %v", err)
		return
	}
}
...
func addSQLiteEvent(ctx context.Context, tx *sql.Tx, res *withdraw.Response) (err error) {
	...
	if errUUID != nil {
		log.Errorf("Failed to generate UUID: %v", errUUID)
		err = tx.Rollback()
		if err != nil {
			log.Errorf("Rollback failed: %v", err)
		}
...
func GetEventsByExchange(exchange string, limit int) ([]*withdraw.Response, error) {
	...
	if err != nil {
		log.Errorf("UUIDByName err(%v)", err)
		return nil, err
	}
...
func getByColumns(q []qm.QueryMod) ([]*withdraw.Response, error) {
	...
	if repository.GetSQLDialect() == database.DBSQLite3 {
		...
		for x := range v {
			...
			if err != nil {
				log.Errorf("Unable to get exchange name")
				tempUUID, errUUID := uuid.FromString(v[x].ExchangeNameID)
				if errUUID != nil {
					log.Errorf("invalid exchange name UUID for record %v", v[x].ID)
				} else {
			...
			if err != nil {
				log.Errorf("record: %v has an incorrect time format ( %v ) - defaulting to empty time: %v", tempResp.ID, v[x].CreatedAt, err)
			...
			if err != nil {
				log.Errorf("record: %v has an incorrect time format ( %v ) - defaulting to empty time: %v", tempResp.ID, v[x].UpdatedAt, err)
