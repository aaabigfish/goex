...
func (b *Bithumb) Run(ctx context.Context) {
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to set exchange order execution limits. Err: %v",
			b.Name,
			err)
	}
...
// withdrawals
func (b *Bithumb) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (b *Bithumb) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	transactions, err := b.GetUserTransactions(ctx, 0, 0, 3, c, currency.EMPTYCODE)
	if err != nil {
		return nil, err
	}
	resp := make([]goex.WithdrawalHistory, len(transactions.Data))
...
func (b *Bithumb) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
