...
func (b *Bittrex) WsConnect() error {
	...
	endpoint, err := b.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (b *Bittrex) WsSignalRHandshake(ctx context.Context, result interface{}) error {
	endpoint, err := b.API.Endpoints.GetURL(goex.WebsocketSpotSupplementary)
	if err != nil {
		return err
	}
...
func (b *Bittrex) WsAuth(ctx context.Context) error {
	...
	if b.Verbose {
		log.Debugf("WebsocketMgr %s Sending JSON message - %s\n", b.Name, requestString)
	}
	...
	if err != nil {
		log.Warnf("WebsocketMgr %s - Cannot unmarshal into WsAuthResponse (%s)\n", b.Name, string(respRaw))
		return err
	}
	if !response.Response.Success {
		log.Warnf("WebsocketMgr %s - Unable to authenticate (%s)", b.Name, response.Response.ErrorCode)
		b.Websocket.SetCanUseAuthenticatedEndpoints(false)
	}
	return nil
}
...
func (b *Bittrex) wsReadData() {
	...
	for {
		...
		default:
			...
			if resp.Raw == nil {
				log.Warnf("WebsocketMgr %s Received empty message\n", b.Name)
				return
			}
...
func (b *Bittrex) wsDecodeMessage(encodedMessage string, v interface{}) error {
	...
	if err = reader.Close(); err != nil {
		log.Warnf("WebsocketMgr %s wsDecodeMessage: unable to close reader: %s",
			b.Name,
			err,
		)
	}
...
func (b *Bittrex) wsHandleData(respRaw []byte) error {
	...
	err := json.Unmarshal(respRaw, &response)
	if err != nil {
		log.Warnf("WebsocketMgr %s Cannot unmarshal into eventResponse (%s)\n", b.Name, string(respRaw))
		return err
	}
	...
	if response.Response == nil && len(response.Message) == 0 && response.C == "" {
		if b.Verbose {
			log.Warnf("WebsocketMgr %s Received keep-alive (%s)\n", b.Name, string(respRaw))
		}
		return nil
	}
	for i := range response.Message {
		...
		case "heartbeat":
			if b.Verbose {
				log.Warnf("WebsocketMgr %s Received heartbeat\n", b.Name)
			}
		case "authenticationExpiring":
			if b.Verbose {
				log.Debugf("WebsocketMgr %s - Re-authenticating.\n", b.Name)
			}
			err = b.WsAuth(context.TODO())
			if err != nil {
				b.Websocket.DataHandler <- err
