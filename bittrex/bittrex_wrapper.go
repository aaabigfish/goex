...
func (b *Bittrex) SetDefaults() {
	...
	b.Features = goex.Features{
		Supports: goex.FeaturesSupported{
			...
			WithdrawPermissions: goex.AutoWithdrawCryptoWithAPIPermission |
	...
	err = b.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:                   bittrexAPIRestURL,
		goex.WebsocketSpot:              bittrexAPIWSURL,
		goex.WebsocketSpotSupplementary: bittrexAPIWSNegotiationsURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	...
	b.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (b *Bittrex) Setup(exch *config.Exchange) error {
	...
	wsRunningEndpoint, err := b.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (b *Bittrex) Run(ctx context.Context) {
	if b.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s.", b.Name, common.IsEnabled(b.Websocket.IsEnabled()))
		b.PrintEnabledPairs()
	}
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", b.Name, err)
	}
	restURL, err := b.API.Endpoints.GetURL(goex.RestSpot)
	if err != nil {
		log.Errorf("ExchangeSys %s failed to check REST Spot URL. Err: %s", b.Name, err)
	}
	if restURL == bittrexAPIDeprecatedURL {
		err = b.API.Endpoints.SetRunning(goex.RestSpot.String(), bittrexAPIRestURL)
		if err != nil {
			log.Errorf("ExchangeSys %s failed to update deprecated REST Spot URL. Err: %s", b.Name, err)
		}
		b.Config.API.Endpoints[goex.RestSpot.String()] = bittrexAPIRestURL
		log.Warnf("ExchangeSys Deprecated %s REST URL updated from %s to %s", b.Name, bittrexAPIDeprecatedURL, bittrexAPIRestURL)
	}
}
...
// withdrawals
func (b *Bittrex) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	...
	resp := make([]goex.FundingHistory, 0, len(depositData)+len(withdrawalData))
	for x := range depositData {
		resp = append(resp, goex.FundingHistory{
			ExchangeName:    b.Name,
...
func (b *Bittrex) GetWithdrawalsHistory(_ context.Context, _ currency.Code, _ asset.Item) (resp []goex.WithdrawalHistory, err error) {
...
func (b *Bittrex) ConstructOrderDetail(orderData *OrderData) (*order.Detail, error) {
	...
	orderPair, err := currency.NewPairDelimiter(orderData.MarketSymbol,
		format.Delimiter)
	if err != nil {
		log.Errorf("ExchangeSys Exchange %v Func %v Order %v Could not parse currency pair %v",
			b.Name,
			"GetActiveOrders",
			orderData.ID,
			err)
...
func (b *Bittrex) GetActiveOrders(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for i := range orderData {
		...
		if err != nil {
			log.Errorf("ExchangeSys GetActiveOrders - %s - cannot get order side - %s\n", b.Name, err.Error())
		}
...
func (b *Bittrex) GetOrderHistory(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for x := range req.Pairs {
		...
		for i := range orderData {
			...
			if err != nil {
				log.Errorf("ExchangeSys GetActiveOrders - %s - cannot get order status - %s\n", b.Name, err.Error())
			}
...
func (b *Bittrex) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
