...
import (
	...
	"github.com/aaabigfish/goex/currencystate"
...
func (b *Base) SetConfigPairs() error {
	...
	for x := range assetTypes {
		if !exchangeAssets.Contains(assetTypes[x]) {
			log.Warnf("ExchangeSys %s exchange asset type %s unsupported, please manually remove from configuration", b.Name, assetTypes[x])
...
func (b *Base) SetupDefaults(exch *config.Exchange) error {
	...
	if exch.Orderbook.VerificationBypass {
		log.Warnf(
			"ExchangeSys %s orderbook verification has been bypassed via config.",
			b.Name)
	}
...
func (b *Base) EnsureOnePairEnabled() error {
	...
	if !pair.IsEmpty() {
		log.Warnf("ExchangeSys %v had no enabled pairs, %v %v pair has been enabled", b.Name, item, pair)
	}
	return nil
}
...
func (b *Base) UpdatePairs(incoming currency.Pairs, a asset.Item, enabled, force bool) error {
	...
	if force || len(diff.New) != 0 || len(diff.Remove) != 0 || diff.FormatDifference {
		...
		if force {
			log.Debugf("ExchangeSys %s forced update of %s [%v] pairs.", b.Name, updateType, strings.ToUpper(a.String()))
		} else {
			if len(diff.New) > 0 {
				log.Debugf(
					"ExchangeSys %s Updating %s pairs [%v] - Added: %s.\n",
					b.Name,
					updateType,
			...
			if len(diff.Remove) > 0 {
				log.Debugf(
					"ExchangeSys %s Updating %s pairs [%v] - Removed: %s.\n",
					b.Name,
					updateType,
					strings.ToUpper(a.String()),
					diff.Remove)
	...
	if len(enabledPairs) == 0 && len(incoming) > 0 {
		...
		log.Debugf("ExchangeSys %s Enabled pairs missing for %s. Added %s.\n",
			b.Name,
			strings.ToUpper(a.String()),
			randomPair)
	...
	if len(diff.Remove) > 0 {
		log.Debugf("ExchangeSys %s Checked and updated enabled pairs [%v] - Removed: %s.\n",
			b.Name,
			strings.ToUpper(a.String()),
			diff.Remove)
	}
...
func (b *Base) SetAPIURL() error {
	checkInsecureEndpoint := func(endpoint string) {
		...
		log.Warnf(
			"ExchangeSys %s is using HTTP instead of HTTPS or WS instead of WSS [%s] for API functionality, an"+
		...
		for key, val := range b.Config.API.Endpoints {
			...
			if err != nil {
				log.Warnf(
					...
					"ExchangeSys %s: Config cannot match with default endpoint URL: [%s] with key: [%s], please remove or update core support endpoints.",
					b.Name,
					val,
					u)
				continue
			...
			log.Warnf(
				"ExchangeSys %s: Config is overwriting default endpoint URL values from: [%s] to: [%s] for: [%s]",
				b.Name,
				defaultURL,
				val,
				u)
...
func (b *Base) PrintEnabledPairs() {
	for k, v := range b.CurrencyPairs.Pairs {
		log.Infof("ExchangeSys %s Asset type %v:\n\t Enabled pairs: %v",
...
func (b *Base) CheckTransientError(err error) error {
	if _, ok := err.(net.Error); ok {
		log.Warnf(
			"ExchangeSys %s net error captured, will not disable authentication %s",
			b.Name,
			err)
		return nil
	}
...
// supported and/or enabled by the requested goex.
...
func (b *Base) SetSaveTradeDataStatus(enabled bool) {
	...
	if b.Verbose {
		log.Debugf("Trade Set %v 'SaveTradeData' to %v", b.Name, enabled)
	}
}
...
func (b *Base) SetTradeFeedStatus(enabled bool) {
	...
	if b.Verbose {
		log.Debugf("Trade Set %v 'TradeFeed' to %v", b.Name, enabled)
	}
}
...
func (b *Base) SetFillsFeedStatus(enabled bool) {
	...
	if b.Verbose {
		log.Debugf("Trade Set %v 'FillsFeed' to %v", b.Name, enabled)
	}
}
...
func (e *Endpoints) SetRunning(key, val string) error {
	...
	if err != nil {
		log.Warnf(
			"ExchangeSys Could not set custom URL for %s to %s for exchange %s. invalid URI for request.",
			key,
			val,
			e.Exchange)
