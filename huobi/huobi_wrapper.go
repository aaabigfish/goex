...
func (h *HUOBI) SetDefaults() {
	...
	err = h.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:         huobiAPIURL,
		goex.RestFutures:      huobiFuturesURL,
		goex.RestCoinMargined: huobiFuturesURL,
		goex.WebsocketSpot:    wsMarketURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	h.Websocket = stream.New()
	h.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	h.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	h.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (h *HUOBI) Setup(exch *config.Exchange) error {
	...
	wsRunningURL, err := h.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (h *HUOBI) Run(ctx context.Context) {
	...
	if err != nil {
		log.Errorf(
			"ExchangeSys %s Failed to update enabled currencies. Err:%s\n",
			h.Name,
			err)
	}
	...
	if common.StringDataContains(h.BaseCurrencies.Strings(), currency.CNY.String()) {
		...
		if err != nil {
			log.Errorf(
				"ExchangeSys %s failed to get exchange config. %s\n",
				h.Name,
				err)
			return
		}
	...
	if forceUpdate {
		...
		log.Warnf("ExchangeSys "+goex.ResetConfigPairsWarningMessage, h.Name, asset.Spot, enabledPairs)
...
// withdrawals
func (h *HUOBI) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (h *HUOBI) GetWithdrawalsHistory(ctx context.Context, c currency.Code, a asset.Item) ([]goex.WithdrawalHistory, error) {
	if a != asset.Spot {
		return nil, fmt.Errorf("%w %v", asset.ErrNotSupported, a)
	}
	...
	resp := make([]goex.WithdrawalHistory, len(withdrawals.Data))
...
func (h *HUOBI) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
...
func setOrderSideStatusAndType(orderState, requestType string, orderDetail *order.Detail) {
	...
	if orderDetail.Status, err = order.StringToOrderStatus(orderState); err != nil {
		log.Errorf("ExchangeSys %s %v", orderDetail.Exchange, err)
	}
