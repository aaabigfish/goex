...
func (b *Base) StartPortfolioWatcher() {
	...
	log.Debugf("PortfolioMgr PortfolioWatcher started: Have %d entries in portfolio.\n", addrCount)
	for {
		data := b.GetPortfolioGroupedCoin()
		for key, value := range data {
			err := b.UpdatePortfolio(value, key)
			if err != nil {
				log.Errorf("PortfolioMgr PortfolioWatcher error %s for currency %s, val %v\n",
					err,
					key,
					value)
				continue
			...
			log.Debugf("PortfolioMgr PortfolioWatcher: Successfully updated address balance for %s address(es) %s\n",
				key,
				value)
		}
