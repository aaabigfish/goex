...
func TestIsExchangeSupported(t *testing.T) {
	...
	ret := newBase.IsExchangeSupported("BTC Markets", common.BitcoinDonationAddress)
	if !ret {
		t.Fatal("expected IsExchangeSupported() to return true")
	}
	ret = newBase.IsExchangeSupported("Kraken", common.BitcoinDonationAddress)
	if ret {
...
func TestIsColdStorage(t *testing.T) {
	t.Parallel()
	newBase := seedPortFolioForTest(t)
	ret := newBase.IsColdStorage(common.BitcoinDonationAddress)
	if !ret {
...
func TestIsWhiteListed(t *testing.T) {
	...
	ret := b.IsWhiteListed(common.BitcoinDonationAddress)
	if !ret {
...
func seedPortFolioForTest(t *testing.T) *Base {
	...
	err := newBase.AddAddress(common.BitcoinDonationAddress, "test", currency.BTC, 1500)
	if err != nil {
