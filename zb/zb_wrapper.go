...
func (z *ZB) SetDefaults() {
	...
	err = z.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:              zbTradeURL,
		goex.RestSpotSupplementary: zbMarketURL,
		goex.WebsocketSpot:         zbWebsocketAPI,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	z.Websocket = stream.New()
	z.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	z.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
}
...
func (z *ZB) Setup(exch *config.Exchange) error {
	...
	wsRunningURL, err := z.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (z *ZB) Run(ctx context.Context) {
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", z.Name, err)
	}
}
...
// withdrawals
func (z *ZB) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	...
	var records []goex.FundingHistory
	for x := range currs {
		totalPages := int64(1)
		for y := int64(0); y < totalPages; y++ {
			...
			for i := range deposits.List {
				...
				records = append(records, goex.FundingHistory{
	...
	for x := range currs {
		var resp []goex.WithdrawalHistory
...
func (z *ZB) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	totalPages := int64(1)
	var records []goex.WithdrawalHistory
	for y := int64(0); y < totalPages; y++ {
		...
		for i := range withdrawals.List {
			...
			records = append(records, goex.WithdrawalHistory{
				Status:          status,
...
func (z *ZB) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
