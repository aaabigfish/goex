...
import (
	...
	"github.com/aaabigfish/goex/common/timedmutex"
	"github.com/aaabigfish/goex/mock"
	"github.com/aaabigfish/goex/nonce"
...
func (i *Item) validateRequest(ctx context.Context, r *Requester) (*http.Request, error) {
	...
	if i.HTTPDebugging {
		...
		log.Debugf("RequestSys DumpRequest:\n%s", dump)
	}
...
func (r *Requester) doRequest(ctx context.Context, endpoint EndpointLimit, newRequest Generate) error {
	for attempt := 1; ; attempt++ {
		...
		if verbose {
			log.Debugf("RequestSys %s attempt %d request path: %s", r.name, attempt, p.Path)
			for k, d := range req.Header {
				log.Debugf("RequestSys %s request header [%s]: %s", r.name, k, d)
			}
			log.Debugf("RequestSys %s request type: %s", r.name, p.Method)
			if p.Body != nil {
				log.Debugf("RequestSys %s request body: %v", r.name, p.Body)
			}
		}
		...
		if retry, checkErr := r.retryPolicy(resp, err); checkErr != nil {
			...
			if verbose {
				log.Errorf(
					"RequestSys  %s request has failed. Retrying request in %s, attempt %d",
					r.name,
					delay,
					attempt)
			}
		...
		if p.HTTPDebugging {
			...
			if err != nil {
				log.Errorf("RequestSys DumpResponse invalid response: %v:", dumpErr)
			}
			log.Debugf("RequestSys DumpResponse Headers (%v):\n%s", p.Path, dump)
			log.Debugf("RequestSys DumpResponse Body (%v):\n %s", p.Path, string(contents))
		}

		err = resp.Body.Close()
		if err != nil {
			log.Errorf(
				"RequestSys %s failed to close request body %s",
				r.name,
				err)
		}
		if verbose {
			log.Debugf(
				"RequestSys HTTP status: %s, Code: %v",
				resp.Status,
				resp.StatusCode)
			if !p.HTTPDebugging {
				log.Debugf(
					"RequestSys %s raw response: %s",
					r.name,
					string(contents))
			}
		}
...
func (r *Requester) drainBody(body io.ReadCloser) {
	if _, err := io.Copy(io.Discard, io.LimitReader(body, drainBodyLimit)); err != nil {
		log.Errorf(
			"RequestSys %s failed to drain request body %s",
			r.name,
			err)
	}
