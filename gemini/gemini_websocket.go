...
func (g *Gemini) WsConnect() error {
	...
	if g.Websocket.CanUseAuthenticatedEndpoints() {
		...
		if err != nil {
			log.Errorf("ExchangeSys %v - websocket authentication failed: %v\n", g.Name, err)
...
func (g *Gemini) WsAuth(ctx context.Context, dialer *websocket.Dialer) error {
	...
	wsEndpoint, err := g.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
		...
		case <-g.Websocket.ShutdownC:
			...
			case resp := <-comms:
				...
				if err != nil {
					...
					default:
						log.Errorf("WebsocketMgr %s websocket handle data error: %v", g.Name, err)
					}
				}
			default:
			}
