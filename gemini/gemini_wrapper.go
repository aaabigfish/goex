...
func (g *Gemini) SetDefaults() {
	...
	g.Features = goex.Features{
		Supports: goex.FeaturesSupported{
			...
			WithdrawPermissions: goex.AutoWithdrawCryptoWithAPIPermission |
				goex.AutoWithdrawCryptoWithSetup |
				goex.WithdrawFiatViaWebsiteOnly,
		},
	...
	err = g.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      geminiAPIURL,
		goex.WebsocketSpot: geminiWebsocketEndpoint,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	g.Websocket = stream.New()
	g.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	g.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	g.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (g *Gemini) Setup(exch *config.Exchange) error {
	...
	if exch.UseSandbox {
		err = g.API.Endpoints.SetRunning(goex.RestSpot.String(), geminiSandboxAPIURL)
		if err != nil {
			log.Errorf("ExchangeSys err(%v)", err)
		}
	}

	wsRunningURL, err := g.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (g *Gemini) Run(ctx context.Context) {
	...
	if !g.BypassConfigFormatUpgrades {
		...
		if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
			...
			if err != nil {
				...
				log.Warnf("ExchangeSys "+goex.ResetConfigPairsWarningMessage, g.Name, asset.Spot, enabledPairs)
				forceUpdate = true
	...
	if err := g.UpdateOrderExecutionLimits(ctx, asset.Spot); err != nil {
		log.Errorf("ExchangeSys %s failed to set exchange order execution limits. Err: %v", g.Name, err)
	}
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", g.Name, err)
	}
}
...
// withdrawals
func (g *Gemini) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	transfers, err := g.Transfers(ctx, currency.EMPTYCODE, time.Time{}, 50, "", false)
	if err != nil {
		return nil, err
	}
	resp := make([]goex.FundingHistory, len(transfers))
	for i := range transfers {
		resp[i] = goex.FundingHistory{
...
func (g *Gemini) GetWithdrawalsHistory(ctx context.Context, c currency.Code, a asset.Item) ([]goex.WithdrawalHistory, error) {
	...
	resp := make([]goex.WithdrawalHistory, 0, len(transfers))
	for i := range transfers {
...
func (g *Gemini) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
