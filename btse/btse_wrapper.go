...
func (b *BTSE) SetDefaults() {
	...
	b.Features = goex.Features{
		Supports: goex.FeaturesSupported{
			...
			WithdrawPermissions: goex.NoAPIWithdrawalMethods,
	...
	err = b.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      btseAPIURL,
		goex.RestFutures:   btseAPIURL,
		goex.WebsocketSpot: btseWebsocket,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
func (b *BTSE) Setup(exch *config.Exchange) error {
	...
	wsRunningURL, err := b.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (b *BTSE) Run(ctx context.Context) {
	...
	if err != nil {
		log.Errorf(
			"ExchangeSys %s Failed to update tradable pairs. Error: %s", b.Name, err)
	}
}
...
// withdrawals
func (b *BTSE) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
...
func (b *BTSE) GetWithdrawalsHistory(_ context.Context, _ currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	return nil, common.ErrFunctionNotSupported
}
...
func (b *BTSE) GetOrderInfo(ctx context.Context, orderID string, _ currency.Pair, _ asset.Item) (*order.Detail, error) {
	...
	for i := range o {
		...
		od.Pair, err = currency.NewPairDelimiter(o[i].Symbol,
			format.Delimiter)
		if err != nil {
			log.Errorf("ExchangeSys %s GetOrderInfo unable to parse currency pair: %s\n", b.Name, err)
		}
		od.Exchange = b.Name
		od.Amount = o[i].Size
		...
		for i := range th {
			...
			if err != nil {
				log.Errorf(
					"ExchangeSys %s GetOrderInfo unable to parse time: %s\n", b.Name, err)
			}
	...
	for x := range req.Pairs {
		...
		for i := range resp {
			...
			p, err := currency.NewPairDelimiter(resp[i].Symbol,
				format.Delimiter)
			if err != nil {
				log.Errorf("ExchangeSys %s GetActiveOrders unable to parse currency pair: %s\n", b.Name, err)
			}
			...
			fills, err := b.TradeHistory(ctx,
				...
				"", resp[i].OrderID)
			if err != nil {
				log.Errorf("ExchangeSys %s: Unable to get order fills for orderID %s", b.Name, resp[i].OrderID)
				continue
			}
			...
			for i := range fills {
				...
				if err != nil {
					log.Errorf("ExchangeSys %s GetActiveOrders unable to parse time: %s\n", b.Name, err)
				}
				var orderSide order.Side
...
func (b *BTSE) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
