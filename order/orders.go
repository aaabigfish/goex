...
import (
	...
	"github.com/aaabigfish/goex/validate"
...
func (s *SubmitResponse) AdjustBaseAmount(a float64) error {
	...
	log.Warnf("ExchangeSys Exchange %s: has adjusted OrderID: %s requested base amount from %v to %v",
		s.Exchange,
		s.OrderID,
		s.Amount,
		a)
...
func (s *SubmitResponse) AdjustQuoteAmount(a float64) error {
	...
	// Warning because amounts should conform to exchange requirements prior to
	// call but this is not fatal.
	log.Warnf("ExchangeSys Exchange %s: has adjusted OrderID: %s requested quote amount from %v to %v",
		s.Exchange,
		s.OrderID,
		s.Amount,
		a)
