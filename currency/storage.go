...
import (
	...
	"github.com/aaabigfish/goex/currency/coinmarketcap"
	"github.com/aaabigfish/goex/currency/forexprovider"
	"github.com/aaabigfish/goex/currency/forexprovider/base"
...
func (s *Storage) SetDefaults() {
	...
	if err != nil {
		log.Errorf("Currency Currency Storage: Setting default fiat currencies error: %s", err)
	}

	err = s.SetStableCoins(stables)
	if err != nil {
		log.Errorf("Currency Currency Storage: Setting default stable currencies error: %s", err)
	}

	err = s.SetDefaultCryptocurrencies(Currencies{BTC, LTC, ETH, DOGE, DASH, XRP, XMR, USDT, UST})
	if err != nil {
		log.Errorf("Currency Currency Storage: Setting default cryptocurrencies error: %s", err)
	}
...
func (s *Storage) RunUpdater(overrides BotOverrides, settings *Config, filePath string) error {
	...
	log.Debugf("Currency Fiat display currency: %s.\n", s.baseCurrency)
	var err error
	if overrides.Coinmarketcap {
		if settings.CryptocurrencyProvider.APIKey != "" &&
			settings.CryptocurrencyProvider.APIKey != "Key" {
			log.Debugf("Currency Setting up currency analysis system with Coinmarketcap...")
			s.currencyAnalysis, err = coinmarketcap.NewFromSettings(coinmarketcap.Settings(settings.CryptocurrencyProvider))
			if err != nil {
				log.Errorf("Currency Unable to setup CoinMarketCap analysis. Error: %s", err)
			}
		} else {
			log.Warnf("Currency %s API key not set, disabling. Please set this in your config.json file\n",
	...
	for i := range settings.ForexProviders {
		...
		if settings.ForexProviders[i].Name != DefaultForexProviderExchangeRatesAPI {
			if settings.ForexProviders[i].APIKey == "" || settings.ForexProviders[i].APIKey == "Key" {
				log.Warnf("Currency %s forex provider API key not set, disabling. Please set this in your config.json file\n",
			...
			if settings.ForexProviders[i].APIKeyLvl == -1 && settings.ForexProviders[i].Name != "ExchangeRates" {
				log.Warnf("Currency %s APIKey level not set, functionality is limited. Please review this in your config.json file\n",
					settings.ForexProviders[i].Name)
			}
		}
		...
		if settings.ForexProviders[i].PrimaryProvider {
			if primaryProvider {
				log.Warnf("Currency %s disabling primary provider, multiple primarys found. Please review providers in your config.json file\n",
					settings.ForexProviders[i].Name)
				settings.ForexProviders[i].PrimaryProvider = false
			} else {
	...
	if len(fxSettings) == 0 {
		log.Warnf("Currency No foreign exchange providers enabled, setting default provider...")
		for x := range settings.ForexProviders {
			...
			primaryProvider = true
			log.Warnf("Currency No valid foreign exchange providers configured. Defaulting to %s.", DefaultForexProviderExchangeRatesAPI)
	...
	if !primaryProvider {
		for x := range settings.ForexProviders {
			if settings.ForexProviders[x].Name == fxSettings[0].Name {
				...
				log.Warnf("Currency No primary foreign exchange provider set. Defaulting to %s.", fxSettings[0].Name)
				break
			}
		}
	}
	...
	log.Debugf("Currency Using primary foreign exchange provider %s\n",
		s.fiatExchangeMarkets.Primary.Provider.GetName())

	for i := range s.fiatExchangeMarkets.Support {
		log.Debugf("Currency Supporting foreign exchange provider %s\n",
...
func (s *Storage) ForeignExchangeUpdater() {
	defer s.wg.Done()
	log.Debugf("Currency Foreign exchange updater started, seeding FX rate list...")

	err := s.SeedCurrencyAnalysisData()
	if err != nil {
		log.Errorf("Currency err(%v)", err)
	}
...
func (s *Storage) FetchCurrencyAnalysisData() error {
	if s.currencyAnalysis == nil {
		log.Warnf("Currency analysis system offline, please set api keys for coinmarketcap if you wish to use this feature.")
