...
func (c *ConversionRates) Register(from, to Code) (Conversion, error) {
	...
	p, ok := c.m[from.Item][to.Item]
	if !ok {
		log.Errorf("Global currency conversion rate not found from %s to %s", from, to)
		return Conversion{}, errors.New("no rate found")
	}

	i, ok := c.m[to.Item][from.Item]
	if !ok {
		log.Errorf("Global currency conversion inversion rate not found from %s to %s", to, from)
		return Conversion{}, errors.New("no rate found")
	}
...
func (c *ConversionRates) Update(m map[string]float64) error {
	...
	if storage.IsVerbose() {
		log.Debug("Global Conversion rates are being updated.")
	}
	...
	for _, base := range list {
		for _, term := range list {
			...
			if !ok {
				...
				if storage.IsVerbose() {
					log.Debugf("Global Conversion from %s to %s deriving cross rate value %f", base, term, crossRate)
				}
