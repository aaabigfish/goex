...
func (ok *Okx) SetDefaults() {
	...
	err = ok.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      okxAPIURL,
		goex.WebsocketSpot: okxAPIWebsocketPublicURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	...
	ok.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (ok *Okx) Setup(exch *config.Exchange) error {
	...
	wsRunningEndpoint, err := ok.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (ok *Okx) Run(ctx context.Context) {
	if ok.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s.", ok.Name,
	...
	for i := range assetTypes {
		if err := ok.UpdateOrderExecutionLimits(ctx, assetTypes[i]); err != nil {
			log.Errorf("ExchangeSys %s failed to set exchange order execution limits. Err: %v", ok.Name, err)
		}
	}

	if ok.GetEnabledFeatures().AutoPairUpdates {
		if err := ok.UpdateTradablePairs(ctx, false); err != nil {
			log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", ok.Name, err)
		}
	}
}
...
func (ok *Okx) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	...
	resp := make([]goex.FundingHistory, 0, len(depositHistories)+len(withdrawalHistories))
...
func (ok *Okx) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	...
	for {
		...
		for i := range orderList {
			...
			for j := range req.Pairs {
				...
				orderStatus, err = order.StringToOrderStatus(strings.ToUpper(orderList[i].State))
				if err != nil {
					log.Errorf("ExchangeSys %s %v", ok.Name, err)
				}
...
func (ok *Okx) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
		return 0, fmt.Errorf("%T %w", feeBuilder, common.ErrNilPointer)
	}
	if !ok.AreCredentialsValid(ctx) && feeBuilder.FeeType == goex.CryptocurrencyTradeFee {
