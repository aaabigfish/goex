...
func (o *Okcoin) SetDefaults() {
	...
	err = o.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      okcoinAPIURL,
		goex.WebsocketSpot: okcoinWebsocketURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	o.Websocket = stream.New()
	o.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	o.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	o.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (o *Okcoin) Setup(exch *config.Exchange) error {
	...
	wsEndpoint, err := o.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (o *Okcoin) Run(ctx context.Context) {
	if o.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s.", o.Name, common.IsEnabled(o.Websocket.IsEnabled()))
		o.PrintEnabledPairs()
	}
		...
		for _, a := range o.GetAssetTypes(true) {
			...
			if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
				...
				p, err = currency.NewPairsFromStrings([]string{currency.BTC.String() +
					...
					currency.USD.String()})
				if err != nil {
					log.Errorf("ExchangeSys %s failed to update currencies.\n", o.Name)
				} else {
					log.Warnf("ExchangeSys "+goex.ResetConfigPairsWarningMessage, o.Name, a, p)
					forceUpdate = true
...
// withdrawals
func (o *Okcoin) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	...
	resp := make([]goex.FundingHistory, len(accountDepositHistory)+len(accountWithdrawlHistory))
	...
	for i := range accountWithdrawlHistory {
		...
		resp[len(accountDepositHistory)+i] = goex.FundingHistory{
...
func (o *Okcoin) GetOrderInfo(ctx context.Context, orderID string, pair currency.Pair, assetType asset.Item) (*order.Detail, error) {
	...
	if err != nil {
		log.Errorf("ExchangeSys %s %v", o.Name, err)
	}
...
func (o *Okcoin) WithdrawCryptocurrencyFunds(ctx context.Context, withdrawRequest *withdraw.Request) (*withdraw.ExchangeResponse, error) {
	...
	if param.TransactionFee == 0 {
		param.TransactionFee, err = o.GetFee(ctx, &goex.FeeBuilder{
			FeeType: goex.CryptocurrencyWithdrawalFee,
			Amount:  param.Amount,
		})
		if err != nil {
			return nil, err
...
func (o *Okcoin) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
	if c.IsFiatCurrency() {
	...
	wHistories := make([]goex.WithdrawalHistory, len(withdrawals))
	for x := range withdrawals {
		...
		wHistories[x] = goex.WithdrawalHistory{
			Status:          orderStatus,
...
func (o *Okcoin) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
