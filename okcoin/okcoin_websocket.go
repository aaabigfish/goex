...
func (o *Okcoin) WsConnect() error {
	...
	if o.Verbose {
		log.Debugf("ExchangeSys Successful connection to %v\n",
...
func (o *Okcoin) WsHandleData(respRaw []byte) error {
	...
	if len(dataResponse.Data) > 0 {
		...
		case wsStatus:
			...
			for x := range resp.Data {
				...
				log.Warnf("ExchangeSys " + systemStatus)
			}
	...
	if err == nil && eventResponse.Event != "" {
		...
		case "error":
			if o.Verbose {
				log.Debugf("ExchangeSys ", o.Name+" - "+eventResponse.Event+" on channel: "+eventResponse.Channel)
			}
		}
	}
	return nil
