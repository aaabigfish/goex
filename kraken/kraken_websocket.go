...
func (k *Kraken) WsConnect() error {
	...
	if k.IsWebsocketAuthenticationSupported() {
		...
		if err != nil {
			...
			log.Errorf("ExchangeSys %v - authentication failed: %v\n", k.Name, err)
		} else {
			err = k.Websocket.AuthConn.Dial(&dialer, http.Header{})
			if err != nil {
				k.Websocket.SetCanUseAuthenticatedEndpoints(false)
				log.Errorf("ExchangeSys %v - failed to connect to authenticated endpoint: %v\n", k.Name, err)
			} else {
				...
				if err != nil {
					log.Errorf("ExchangeSys %v - failed setup ping handler for auth connection. Websocket may disconnect unexpectedly. %v\n", k.Name, err)
				}
			}
		}
	}
	...
	if err != nil {
		log.Errorf(
			"ExchangeSys %v - failed setup ping handler. Websocket may disconnect unexpectedly. %v\n",
			k.Name,
			err)
	}
	return nil
		...
		case <-k.Websocket.ShutdownC:
			...
			case resp := <-comms:
				...
				if err != nil {
					...
					default:
						log.Errorf("WebsocketMgr %s websocket handle data error: %v", k.Name, err)
					}
				}
			default:
			}
		...
		if event, ok := eventResponse["event"]; ok {
			...
			case krakenWsSystemStatus:
				...
				if systemStatus.Version > krakenWSSupportedVersion {
					log.Warnf(
						"ExchangeSys %v New version of Websocket API released. Was %v Now %v",
						k.Name,
...
func (k *Kraken) addNewSubscriptionChannelData(response *wsSubscription) {
	...
	if response.Pair != "" {
		...
		if err != nil {
			log.Errorf("ExchangeSys %s exchange error: %s", k.Name, err)
			return
		}
...
func (k *Kraken) wsProcessSpread(channelData *WebsocketChannelData, data []interface{}) {
	...
	if k.Verbose {
		log.Debugf(
			"ExchangeSys %v Spread data for '%v' received. Best bid: '%v' Best ask: '%v' Time: '%v', Bid volume '%v', Ask volume '%v'",
			k.Name,
			channelData.Pair,
			bestBid,
			bestAsk,
	...
	if asksExist || bidsExist {
		...
		if err != nil {
			...
			go func(resub *stream.ChannelSubscription) {
				...
				if errResub != nil {
					log.Errorf("WebsocketMgr resubscription failure for %v: %v", resub, errResub)
				}
