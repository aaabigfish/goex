...
func (k *Kraken) SetDefaults() {
	...
	k.Features = goex.Features{
		Supports: goex.FeaturesSupported{
			...
			WithdrawPermissions: goex.AutoWithdrawCryptoWithSetup |
				goex.WithdrawCryptoWith2FA |
				goex.AutoWithdrawFiatWithSetup |
				goex.WithdrawFiatWith2FA,
	...
	err = k.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:                 krakenAPIURL,
		goex.RestFutures:              krakenFuturesURL,
		goex.WebsocketSpot:            krakenWSURL,
		goex.RestFuturesSupplementary: krakenFuturesSupplementaryURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	k.Websocket = stream.New()
	k.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	k.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	k.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (k *Kraken) Setup(exch *config.Exchange) error {
	...
	wsRunningURL, err := k.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (k *Kraken) Run(ctx context.Context) {
	...
	if !k.BypassConfigFormatUpgrades {
		...
		if err != nil {
			log.Errorf(
				"ExchangeSys %s failed to update tradable pairs. Err: %s",
				k.Name,
				err)
			return
		}
		...
		if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
			...
			if err != nil {
				log.Errorf(
					"ExchangeSys %s failed to update currencies. Err: %s\n",
					k.Name,
					err)
			} else {
				log.Warnf("ExchangeSys "+goex.ResetConfigPairsWarningMessage, k.Name, asset.UseDefault(), p)
				forceUpdate = true
	...
	for _, a := range k.GetAssetTypes(true) {
		if err := k.UpdateOrderExecutionLimits(ctx, a); err != nil && err != common.ErrNotYetImplemented {
			log.Errorf("ExchangeSys err(%v)", err.Error())
		}
	}
}
...
func (k *Kraken) fetchSpotPairInfo(ctx context.Context) (map[currency.Pair]*AssetPairs, error) {
	...
	for _, info := range pairInfo {
		...
		if base == "" {
			log.Warnf(
				"ExchangeSys %s unable to lookup altname for base currency %s",
				k.Name,
				info.Base)
			continue
		}
		...
		if quote == "" {
			log.Warnf(
				"ExchangeSys %s unable to lookup altname for quote currency %s",
				k.Name,
				info.Quote)
			continue
		}
	...
	case asset.Spot:
		...
		for key := range bal {
			...
			if translatedCurrency == "" {
				log.Warnf("ExchangeSys %s unable to translate currency: %s\n",
					k.Name,
					key)
				continue
			}
...
// withdrawals
func (k *Kraken) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (k *Kraken) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
...
func (k *Kraken) GetOrderInfo(ctx context.Context, orderID string, _ currency.Pair, assetType asset.Item) (*order.Detail, error) {
	...
	case asset.Spot:
		...
		if err != nil {
			log.Errorf("ExchangeSys %s %v", k.Name, err)
		}
...
// Populate goex.WithdrawRequest.TradePassword with withdrawal key name, as set up on your account
...
func (k *Kraken) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
...
func (k *Kraken) GetFuturesContractDetails(ctx context.Context, item asset.Item) ([]futures.Contract, error) {
	...
	for i := range result.Instruments {
		...
		if usdIndex <= 0 {
			log.Warnf("ExchangeSys %v unable to find USD index in %v to process contract", k.Name, underlyingStr)
			continue
		}
