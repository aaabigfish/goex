...
// AlphaPoint goex. To begin from the most recent trade, set startIndex to
// 0 (default: 0)
...
func (a *Alphapoint) GetTrades(ctx context.Context, currencyPair string, startIndex, count int) (Trades, error) {
	...
	err := a.SendHTTPRequest(ctx,
		goex.RestSpot, http.MethodPost, alphapointTrades, req, &response)
	if err != nil {
		return response, err
	}
	if !response.IsAccepted {
...
func (a *Alphapoint) GetOrderbook(ctx context.Context, currencyPair string) (Orderbook, error) {
	...
	err := a.SendHTTPRequest(ctx,
		goex.RestSpot, http.MethodPost, alphapointOrderbook, req, &response)
	if err != nil {
		return response, err
	}
	if !response.IsAccepted {
...
func (a *Alphapoint) GetProductPairs(ctx context.Context) (ProductPairs, error) {
	...
	err := a.SendHTTPRequest(ctx,
		goex.RestSpot, http.MethodPost, alphapointProductPairs, nil, &response)
	if err != nil {
		return response, err
	}
	if !response.IsAccepted {
...
func (a *Alphapoint) GetProducts(ctx context.Context) (Products, error) {
	...
	err := a.SendHTTPRequest(ctx,
		goex.RestSpot, http.MethodPost, alphapointProducts, nil, &response)
	if err != nil {
		return response, err
	}
	if !response.IsAccepted {
...
func (a *Alphapoint) CreateAccount(ctx context.Context, firstName, lastName, email, phone, password string) error {
	...
	err := a.SendAuthenticatedHTTPRequest(ctx,
		goex.RestSpot, http.MethodPost, alphapointCreateAccount, req, &response)
	if err != nil {
...
func (a *Alphapoint) SendHTTPRequest(_ context.Context, ep goex.URL, method, path string, data map[string]interface{}, result interface{}) error {
...
func (a *Alphapoint) SendAuthenticatedHTTPRequest(ctx context.Context, ep goex.URL, method, path string, data map[string]interface{}, result interface{}) error {
