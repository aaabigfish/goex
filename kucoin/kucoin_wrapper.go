...
func (ku *Kucoin) SetDefaults() {
	...
	err = ku.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      kucoinAPIURL,
		goex.RestFutures:   kucoinFuturesAPIURL,
		goex.WebsocketSpot: kucoinWebsocketURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
	ku.Websocket = stream.New()
	ku.WebsocketResponseMaxLimit = goex.DefaultWebsocketResponseMaxLimit
	ku.WebsocketResponseCheckTimeout = goex.DefaultWebsocketResponseCheckTimeout
	ku.WebsocketOrderbookBufferLimit = goex.DefaultWebsocketOrderbookBufferLimit
}
...
func (ku *Kucoin) Setup(exch *config.Exchange) error {
	...
	wsRunningEndpoint, err := ku.API.Endpoints.GetURL(goex.WebsocketSpot)
	if err != nil {
		return err
	}
...
func (ku *Kucoin) Run(ctx context.Context) {
	if ku.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s.", ku.Name, common.IsEnabled(ku.Websocket.IsEnabled()))
	...
	err := ku.UpdateTradablePairs(ctx, true)
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", ku.Name, err)
	}
}
...
// withdrawals
func (ku *Kucoin) GetAccountFundingHistory(ctx context.Context) ([]goex.FundingHistory, error) {
	...
	fundingData := make([]goex.FundingHistory, len(withdrawalsData.Items)+len(depositsData.Items))
	for x := range depositsData.Items {
		fundingData[x] = goex.FundingHistory{
	...
	for x := range withdrawalsData.Items {
		fundingData[length+x] = goex.FundingHistory{
...
func (ku *Kucoin) GetWithdrawalsHistory(ctx context.Context, c currency.Code, a asset.Item) ([]goex.WithdrawalHistory, error) {
	...
	case asset.Spot:
		...
		resp := make([]goex.WithdrawalHistory, len(withdrawals.Items))
		for x := range withdrawals.Items {
			resp[x] = goex.WithdrawalHistory{
	...
	case asset.Futures:
		...
		resp := make([]goex.WithdrawalHistory, len(futuresWithdrawals.Items))
		for y := range futuresWithdrawals.Items {
			resp[y] = goex.WithdrawalHistory{
...
func (ku *Kucoin) GetOrderHistory(ctx context.Context, getOrdersRequest *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	case asset.Futures:
		...
		for i := range orders {
			...
			if err != nil {
				log.Errorf("ExchangeSys %s %v", ku.Name, err)
			}
			orders = append(orders, order.Detail{
...
func (ku *Kucoin) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
