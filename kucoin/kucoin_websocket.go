...
func (ku *Kucoin) GetInstanceServers(ctx context.Context) (*WSInstanceServers, error) {
	...
	return &(response.Data), ku.SendPayload(ctx, request.Unset, func() (*request.Item, error) {
		endpointPath, err := ku.API.Endpoints.GetURL(goex.RestSpot)
		if err != nil {
			return nil, err
		}
		return &request.Item{
...
func (ku *Kucoin) GetAuthenticatedInstanceServers(ctx context.Context) (*WSInstanceServers, error) {
	...
	err := ku.SendAuthHTTPRequest(ctx, goex.RestSpot, defaultSpotEPL, http.MethodPost, privateBullets, nil, &response)
	if err != nil && strings.Contains(err.Error(), "400003") {
		return response.Data, ku.SendAuthHTTPRequest(ctx, goex.RestFutures, defaultFuturesEPL, http.MethodPost, privateBullets, nil, &response)
	}
...
func (ku *Kucoin) applyBufferUpdate(pair currency.Pair, assetType asset.Item) error {
	...
	if needsFetching {
		if ku.Verbose {
			log.Debugf("WebsocketMgr %s Orderbook: Fetching via REST\n", ku.Name)
		}
	...
	if err != nil {
		log.Errorf(
			...
			"WebsocketMgr %s error fetching recent orderbook when applying updates: %s\n",
			ku.Name,
			err)
	}
	...
	if recent != nil {
		...
		if err != nil {
			log.Errorf(
				...
				"WebsocketMgr %s error processing update - initiating new orderbook sync via REST: %s\n",
				ku.Name,
				err)
	...
	go func() {
		...
		for {
			...
			case j := <-ku.obm.jobs:
				...
				if err != nil {
					log.Errorf(
						"WebsocketMgr %s processing websocket orderbook error %v",
						ku.Name, err)
				}
			}
		}
...
func (ku *Kucoin) flushAndCleanup(p currency.Pair, assetType asset.Item) {
	...
	if errClean != nil {
		log.Errorf("WebsocketMgr %s flushing websocket error: %v", ku.Name, errClean)
	}
	errClean = ku.obm.cleanup(p, assetType)
	if errClean != nil {
		log.Errorf("WebsocketMgr %s cleanup websocket error: %v", ku.Name, errClean)
	}
}
