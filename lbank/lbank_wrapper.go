...
func (l *Lbank) SetDefaults() {
	...
	err = l.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot: lbankAPIURL,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
func (l *Lbank) Setup(exch *config.Exchange) error {
	...
	if l.API.AuthenticatedSupport {
		...
		if err != nil {
			...
			log.Errorf("ExchangeSys %s couldn't load private key, setting authenticated support to false", l.Name)
		}
	}
	return nil
}
...
func (l *Lbank) Run(ctx context.Context) {
	...
	if err != nil {
		log.Errorf("ExchangeSys %s failed to update tradable pairs. Err: %s", l.Name, err)
	}
}
...
// withdrawals
func (l *Lbank) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (l *Lbank) GetWithdrawalsHistory(ctx context.Context, c currency.Code, a asset.Item) ([]goex.WithdrawalHistory, error) {
	...
	resp := make([]goex.WithdrawalHistory, len(withdrawalRecords.List))
...
func (l *Lbank) GetOrderInfo(ctx context.Context, orderID string, _ currency.Pair, _ asset.Item) (*order.Detail, error) {
	...
	for key, val := range orderIDs {
		for i := range val {
			...
			resp.Fee, err = l.GetFeeByType(ctx, &goex.FeeBuilder{
...
func (l *Lbank) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
	...
	if feeBuilder.FeeType == goex.CryptocurrencyWithdrawalFee {
...
func (l *Lbank) GetStatus(status int64) order.Status {
	...
	default:
		log.Errorf("Global %s Unhandled Order Status '%v'", l.GetName(), status)
	}
	return oStatus
}
