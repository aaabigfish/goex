...
func (b *Bitfinex) SetDefaults() {
	...
	err = b.API.Endpoints.SetDefaultEndpoints(map[goex.URL]string{
		goex.RestSpot:      bitfinexAPIURLBase,
		goex.WebsocketSpot: publicBitfinexWebsocketEndpoint,
	})
	if err != nil {
		log.Errorf("ExchangeSys err(%v)", err)
	}
...
func (b *Bitfinex) Run(ctx context.Context) {
	if b.Verbose {
		log.Debugf("ExchangeSys %s Websocket: %s.",
			b.Name,
...
// withdrawals
func (b *Bitfinex) GetAccountFundingHistory(_ context.Context) ([]goex.FundingHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (b *Bitfinex) GetWithdrawalsHistory(ctx context.Context, c currency.Code, _ asset.Item) ([]goex.WithdrawalHistory, error) {
...
func (b *Bitfinex) parseOrderToOrderDetail(o *Order) (*order.Detail, error) {
	...
	if err != nil {
		log.Warnf("ExchangeSys %s Unable to convert timestamp '%s', leaving blank",
			b.Name, o.Timestamp)
	}
...
func (b *Bitfinex) GetFeeByType(ctx context.Context, feeBuilder *goex.FeeBuilder) (float64, error) {
	if feeBuilder == nil {
