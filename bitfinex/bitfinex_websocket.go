...
func (b *Bitfinex) WsConnect() error {
	...
	if b.Websocket.CanUseAuthenticatedEndpoints() {
		...
		if err != nil {
			log.Errorf(
				"ExchangeSys %v unable to connect to authenticated Websocket. Error: %s",
				b.Name,
				err)
		...
		if err != nil {
			log.Errorf(
				"ExchangeSys %v - authentication failed: %v\n",
				b.Name,
				err)
			b.Websocket.SetCanUseAuthenticatedEndpoints(false)
		}
		...
		case <-b.Websocket.ShutdownC:
			...
			case resp := <-comms:
				...
				if err != nil {
					...
					default:
						log.Errorf("WebsocketMgr %s websocket handle data error: %v",
							b.Name,
							err)
					}
				}
...
func (b *Bitfinex) WsUpdateOrderbook(p currency.Pair, assetType asset.Item, book []WebsocketBook, channelID int, sequenceNo int64, fundingRate bool) error {
	...
	if checkme.Sequence+1 == sequenceNo {
		...
		if err = validateCRC32(ob, checkme.Token); err != nil {
			log.Errorf("WebsocketMgr %s websocket orderbook update error, will resubscribe orderbook: %v", b.Name, err)
			if suberr := b.resubOrderbook(p, assetType); suberr != nil {
				log.Errorf("ExchangeSys %s error resubscribing orderbook: %v", b.Name, suberr)
			}
			return err
		}
	}
...
func (b *Bitfinex) WsAddSubscriptionChannel(chanID int, channel, symbol string) error {
	...
	if c == nil {
		log.Errorf("ExchangeSys %s Could not find an existing channel subscription: %s Pair: %s ChannelID: %d Asset: %s\n",
			b.Name,
			channel,
			pair,
			chanID,
	...
	if b.Verbose {
		log.Debugf("ExchangeSys %s Subscribed to Channel: %s Pair: %s ChannelID: %d\n",
			b.Name,
			channel,
			pair,
			chanID)
