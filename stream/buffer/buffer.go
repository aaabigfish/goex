...
import (
	...
	"github.com/aaabigfish/goex/common/key"
...
func (w *Orderbook) Update(u *orderbook.Update) error {
	...
	if w.updateIDProgression && u.UpdateID <= book.updateID {
		if w.verbose {
			log.Warnf(
				"WebsocketMgr Exchange %s CurrencyPair: %s AssetType: %s out of order websocket update received",
				w.exchangeName,
				u.Pair,
				u.Asset)
		}
	...
	if err != nil {
		...
		if w.verbose {
			log.Warnf(
				"WebsocketMgr Exchange %s CurrencyPair: %s AssetType: %s underlying book is invalid, cannot apply update.",
				w.exchangeName,
				u.Pair,
				u.Asset)
		}
	...
	if isREST {
		if w.verbose {
			log.Warnf(
				"WebsocketMgr %s for Exchange %s CurrencyPair: %s AssetType: %s consider extending synctimeoutwebsocket",
				errRESTOverwrite,
				w.exchangeName,
				u.Pair,
				u.Asset)
