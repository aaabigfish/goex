	...
	go func() {
		...
		for {
			...
			case d := <-w.DataHandler:
				...
				default:
					log.Warnf(
						"WebsocketMgr %s exchange backlog in websocket processing detected",
						w.exchangeName)
					select {
					case w.ToRoutine <- d:
					case <-w.ShutdownC:
...
func (w *Websocket) connectionMonitor() error {
	...
	go func() {
		...
		for {
			if w.verbose {
				log.Debugf(
					"WebsocketMgr %v websocket: running connection monitor cycle\n",
					w.exchangeName)
			}
			if !w.IsEnabled() {
				if w.verbose {
					log.Debugf(
						"WebsocketMgr %v websocket: connectionMonitor - websocket disabled, shutting down\n",
						w.exchangeName)
				}
				if w.IsConnected() {
					err := w.Shutdown()
				...
				if w.verbose {
					log.Debugf(
						"WebsocketMgr %v websocket: connection monitor exiting\n",
						w.exchangeName)
				}
				timer.Stop()
			...
			case err := <-w.ReadMessageErrors:
				if IsDisconnectionError(err) {
					w.setInit(false)
					log.Warnf(
						"WebsocketMgr %v websocket has been disconnected. Reason: %v",
						w.exchangeName, err)
...
func (w *Websocket) Shutdown() error {
	...
	if w.verbose {
		log.Debugf(
			"WebsocketMgr %v websocket: shutting down websocket\n",
			w.exchangeName)
	}
	...
	if w.verbose {
		log.Debugf(
			"WebsocketMgr %v websocket: completed websocket shutdown\n",
			w.exchangeName)
	}
	return nil
}
	...
	go func() {
		...
		for {
			...
			case <-w.ShutdownC:
				if w.verbose {
					log.Debugf(
						"WebsocketMgr %v websocket: trafficMonitor shutdown message received\n",
						w.exchangeName)
				}
				trafficTimer.Stop()
			...
			case <-trafficTimer.C: // Falls through when timer runs out
				if w.verbose {
					log.Warnf(
						"WebsocketMgr %v websocket: has not received a traffic alert in %v. Reconnecting",
						w.exchangeName,
						w.trafficTimeout)
				}
				trafficTimer.Stop()
				...
				if !w.IsConnecting() && w.IsConnected() {
					...
					if err != nil {
						log.Errorf(
							"WebsocketMgr %v websocket: trafficMonitor shutdown err: %s",
							w.exchangeName, err)
					}
				}
...
func (w *Websocket) CanUseAuthenticatedWebsocketForWrapper() bool {
	if w.IsConnected() && w.CanUseAuthenticatedEndpoints() {
		...
		log.Infof(
			"WebsocketMgr "+WebsocketNotAuthenticatedUsingRest,
			w.exchangeName)
	}
	return false
}
...
func (w *Websocket) SetWebsocketURL(url string, auth, reconnect bool) error {
	...
	if auth {
		...
		if w.verbose {
			log.Debugf(
				"WebsocketMgr %s websocket: setting authenticated websocket URL: %s\n",
				w.exchangeName,
				url)
		}
		...
		if w.verbose {
			log.Debugf(
				"WebsocketMgr %s websocket: setting unauthenticated websocket URL: %s\n",
				w.exchangeName,
				url)
		}
	...
	if w.IsConnected() && reconnect {
		log.Debugf(
			"WebsocketMgr %s websocket: flushing websocket connection to %s\n",
			w.exchangeName,
			url)
		return w.Shutdown()
	}
...
func (w *Websocket) SetProxyAddress(proxyAddr string) error {
	if proxyAddr != "" {
		...
		log.Debugf(
			"ExchangeSys %s websocket: setting websocket proxy: %s\n",
			w.exchangeName,
			proxyAddr)
	} else {
		log.Debugf(
			"ExchangeSys %s websocket: removing websocket proxy\n",
			w.exchangeName)
	}
