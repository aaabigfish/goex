...
func (w *WebsocketConnection) Dial(dialer *websocket.Dialer, headers http.Header) error {
	...
	if w.Verbose {
		log.Infof(
			"WebsocketMgr %v Websocket connected to %s\n",
			w.ExchangeName,
			w.URL)
	}
	select {
...
func (w *WebsocketConnection) SendJSONMessage(data interface{}) error {
	...
	if w.Verbose {
		log.Debugf(
			"WebsocketMgr %s websocket connection: sending message to websocket %+v\n",
			w.ExchangeName,
			data)
	}
...
func (w *WebsocketConnection) SendRawMessage(messageType int, message []byte) error {
	...
	if w.Verbose {
		log.Debugf(
			"WebsocketMgr %v websocket connection: sending message [%s]\n",
			w.ExchangeName,
			message)
	}
	if w.RateLimit > 0 {
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if err != nil {
					log.Errorf(
						"WebsocketMgr %v websocket connection: ping handler failed to send message [%s]",
						w.ExchangeName,
						handler.Message)
					return
				}
...
func (w *WebsocketConnection) ReadMessage() Response {
		...
		if IsDisconnectionError(err) {
			if w.setConnectedStatus(false) {
				...
				default:
					...
					log.Warnf(
						"WebsocketMgr %s failed to relay error: %v",
						w.ExchangeName,
						err)
				}
			}
	...
	case websocket.BinaryMessage:
		...
		if err != nil {
			log.Errorf(
				"WebsocketMgr %v websocket connection: parseBinaryResponse error: %v",
				w.ExchangeName,
				err)
			return Response{}
		}
	...
	if w.Verbose {
		log.Debugf(
			"WebsocketMgr %v websocket connection: message received: %v",
			w.ExchangeName,
